	...
	protected:
		...
		LRESULT OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& )
		{
			...
			if( errorCode != 0)
			{
				...
				pLogFont = &logFont;				
				pLogFont->lfHeight = 13;                      // request a 13-pixel-height font
				rtl_strcpy( pLogFont->lfFaceName, LF_FACESIZE, _T("MS Shell Dlg")); 
				//pLogFont = &logFont;
			}
			else
			{
			...
			m_HorizFont.CreateFontIndirect(pLogFont);
		
			// Left Tab Font
			pLogFont->lfOrientation = 900;
			pLogFont->lfEscapement  = 900;
			m_LeftFont.CreateFontIndirect(pLogFont);

			// Right Tab Font	
			pLogFont->lfOrientation = 2700;
			pLogFont->lfEscapement  = 2700;
			m_RightFont.CreateFontIndirect(pLogFont);
