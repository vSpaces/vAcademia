...
// e-mail -> per@gorep.se
// 
// Author -> Per Ghosh (e-mail: per@gorep.se) and Andrey Tanakov (e-mail: tandyru@mail.ru)
//
...
/*! \file gd_HTMLArchive.h
    \brief has template class with functionality to read or store HTML data


*/

#if !defined(AFX_HTMLARCHIVE_H__664C0238_A109_453B_9341_6AF4614AD628__INCLUDED_)
#define AFX_HTMLARCHIVE_H__664C0238_A109_453B_9341_6AF4614AD628__INCLUDED_

#if _MSC_VER > 1000
#pragma once
#endif // _MSC_VER > 1000

#ifndef _GD_HTML_ARCHIVE_H_
   #define _GD_HTML_ARCHIVE_H_
#endif
...
#define _htmlarchive_NullCharacterFound          0x0010
#define _htmlarchive_NullCharacterBeforeEndNode  0x0011
#define _htmlarchive_EndingDoesNotMatchParent    0x0012
#define _htmlarchive_XMLHasEndedBeforeEndOfTags  0x0013

/*! \brief namespace for streaming html
*/
namespace gd_html_stream
{                                      
...
/*! \class CHTMLItem
    \brief item use for implicit convertion of normal C++ datatypes to CHTMLArchive
...
public:
   CHTMLItem() {}
   CHTMLItem( int iVal ) { m_sType = eTI4; m_V.valLong = iVal; }

   CHTMLItem( char chVal ) { m_sType = eTI1; m_V.valChar = chVal; }
   CHTMLItem( unsigned char uchVal ) { m_sType = eTUI1; m_V.valUChar = uchVal; }
   CHTMLItem( short sVal ) { m_sType = eTI2; m_V.valShort = sVal; }
   CHTMLItem( unsigned short usVal ) { m_sType = eTUI2; m_V.valUShort = usVal; }
   CHTMLItem( long lVal ) { m_sType = eTI4; m_V.valLong = lVal; }
   CHTMLItem( unsigned long ulVal ) { m_sType = eTUI4; m_V.valULong = ulVal; }
   CHTMLItem( float fVal ) { m_sType = eTR4; m_V.valFloat = fVal; }
   CHTMLItem( double dVal ) { m_sType = eTR8; m_V.valDouble = dVal; }
   CHTMLItem( const wchar_t* pszValue ) { m_sType = eTSTR; m_V.valpsz = pszValue; }
   CHTMLItem( const gd::CString& string ) { m_sType = eTSTR; m_V.valpsz = (const LPCWSTR)string; }
...
//! value that sets next item that is managed by the CHTMLArchive when streaming data
enum Operation
{
   Child       = 0x0020,
   Attribute   = 0x0040,
...
//! value that sets end operation for CHTMLArchive when streaming data
enum EndOperation
{
   EndNode     = 1,
   End         = 2,
 ...
 * \brief helper object for adding attributes to xml.
 ...
 Simple class used to add attributes to CHTMLArchive.

 \code
CHTMLArchive<CFile> ar( &file, CHTMLArchive<CFile>::eWrite );
ar << Child << "DOCUMENT" 
...
struct attribute
{
   attribute( const wchar_t* pszName, const CHTMLItem& Item ) : m_pszName( pszName ), m_Item( Item ) {}
...
The CHTMLArchive class makes it more easy to store data in this format. It's very fast. It
doesn't check for valid XML (just the basic stuff). This means that you need to
work on XML'files that ar valid.
\p
In order to use CHTMLArchive here are the steps to follow.<br>
1: You need to create a class that with functions for reading and writing. The CHTMLArchive is done
...
<br>
2: The CHTMLArchive process informaiton in blocks. It is possible to specifiy the size of the block.
The reason why CHTMLArchive processes information this way is to enable larger files for processing. 
...
<br>
3: Writing information is could be done by streaming information to the CHTMLArchive object. There 
are one helper class called CHTMLItem. The purpose for this class it to create implicit convertion
...
It is possible to looka ahead for one item using the CHTMLArchive. This is often what you need in order
to process information. If you need to look further then you shold store elements in the application. 
This can't be done in CHTMLArchive.
<br>
<br>
FAQ: 
...
<br>
   ...
   EXAMPLE: (sample on how to use MFC CFile object with CHTMLArchive)
\code
   CFile file;
   file.Open( _U("one.xml"), CFile::modeWrite|CFile::modeCreate );
   CHTMLArchive<CFile> Archive( &file, CHTMLArchive<CFile>::eWrite ); \endcode
<br>
...
A: EXAMPLE: \code
   ...
   CFile file;
   file.Open( _U("one.xml"), CFile::modeWrite|CFile::modeCreate );
   CHTMLArchive<CFile> Archive( &file, CHTMLArchive<CFile>::eWrite );
   i = 1000;
...
\code
   ...
   CHTMLArchive<CFileText> ar( &File, eRead );
   // ...
   // ...
...
public:
   CHTMLArchive( STREAM* pSTREAM, UINT uState = eRead, UINT uBufferSize = 4096, wchar_t* pszBuffer = NULL );
	virtual ~CHTMLArchive();

// Operator
public:
   CHTMLArchive& operator<<(Operation operation) {
      NextItem( operation ); return *this; }
   CHTMLArchive& operator<<(EndOperation operation) {
      EndItem( operation ); return *this; }
   CHTMLArchive& operator<<(const CHTMLItem& item) {
      SetItem( item ); return *this; }
   CHTMLArchive& operator<<(const attribute& a); 
   //! stream char value
   CHTMLArchive& operator>>(char& ch) {
      CHTMLItem item; item.SetType( CHTMLItem::eTI1 ); GetItem( NULL, item ); 
      ch = item; return *this; }
   //! stream unsigned char value
   CHTMLArchive& operator>>(unsigned char& ch) {
      CHTMLItem item; item.SetType( CHTMLItem::eTUI1 ); GetItem( NULL, item ); 
      ch = item; return *this; }
   //! stream short value
   CHTMLArchive& operator>>(short& s) {
      CHTMLItem item; item.SetType( CHTMLItem::eTI2 ); GetItem( NULL, item ); 
      s = item; return *this; }
   //! stream unsigned short value
   CHTMLArchive& operator>>(unsigned short& s) {
      CHTMLItem item; item.SetType( CHTMLItem::eTUI2 ); GetItem( NULL, item ); 
      s = item; return *this; }
   //! stream long value
   CHTMLArchive& operator>>(long& l) {
      CHTMLItem item; item.SetType( CHTMLItem::eTI4 ); GetItem( NULL, item ); 
      l = item; return *this; }
   //! stream unsigned long value
   CHTMLArchive& operator>>(unsigned long& l) {
      CHTMLItem item; item.SetType( CHTMLItem::eTUI4 ); GetItem( NULL, item ); 
      l = item; return *this; }
   //! stream float value
   CHTMLArchive& operator>>(float& f) {
      CHTMLItem item; item.SetType( CHTMLItem::eTR4 ); GetItem( NULL, item ); 
      f = item; return *this; }
   //! stream double value
   CHTMLArchive& operator>>(double& d) {
      CHTMLItem item; item.SetType( CHTMLItem::eTR8 ); GetItem( NULL, item ); 
      d = item; return *this; }
   //! stream character buffer
   CHTMLArchive& operator>>(wchar_t*& psz) {
      CHTMLItem item; item.SetType( CHTMLItem::eTSTR ); GetItem( NULL, item ); 
...
public:
   ...
   void SetItem( const CHTMLItem& item );
   ...
   HRESULT GetItem( const wchar_t* pszName, CHTMLItem& item );
   //! read next node
   bool ReadNext();
   //! read next node
...
// CHECKING
   ...
   //! end the node (WRITE)
   static void EndNode( CHTMLArchive* pThis );
   //! end all nodes (WRITE)
   void End();
...
protected:
   ...
   static bool _CompareText( CHTMLArchive* pThis, wchar_t* pszText, int iLength );

// Attributes
public:
   // STATE
   UINT m_uState;                //!< state for CHTMLArchive
...
template<class STREAM>
CHTMLArchive<STREAM>& CHTMLArchive<STREAM>::operator<<(const attribute& a) {
...
template<class STREAM>
__forceinline void CHTMLArchive<STREAM>::_Out( wchar_t ch ) {
...
template<class STREAM>
wchar_t CHTMLArchive<STREAM>::_In() {
   wchar_t ch;
   ch = *m_pszPosition;
   m_pszPosition++;
...
// construct
template<class STREAM>
CHTMLArchive<STREAM>::CHTMLArchive( STREAM* pSTREAM, UINT uState, UINT uBufferSize, wchar_t* pszBuffer )
{
   _CHECK( pSTREAM != NULL );
...
// destruct
template<class STREAM>
CHTMLArchive<STREAM>::~CHTMLArchive()
{
   Flush();
 ...
 */
template<class STREAM>
UINT CHTMLArchive<STREAM>::GetParentCount()
{
...
template<class STREAM>
__forceinline bool CHTMLArchive<STREAM>::HasChildren() {
   return ( m_uTagState & eHasChild ) ? true : false; }

template<class STREAM>
__forceinline bool CHTMLArchive<STREAM>::HasNextChildren() {
   return ( m_uTagStateNext & eHasChild ) ? true : false; }

/////////////////////////////////////////////////
//! check if it is a end tag
template<class STREAM>
__forceinline bool CHTMLArchive<STREAM>::IsEndTag() {
   return ( m_uTagState & eEndTag ) ? true : false; }

template<class STREAM>
__forceinline bool CHTMLArchive<STREAM>::IsNextEndTag() {
   return ( m_uTagStateNext & eEndTag ) ? true : false; }

/////////////////////////////////////////////////
//! add a new child node
template<class STREAM>
void CHTMLArchive<STREAM>::NewChild( const wchar_t* pszName /*!< name of node that is added*/ )
{
...
/*! create new sibling for node. This means that the created node has the same parent as
   the prevoius node. */
template<class STREAM>
void CHTMLArchive<STREAM>::NewSibling( const wchar_t* pszName /*!< name of the sibling*/ )
{
   _CHECK( m_uState & eWrite );
...
template<class STREAM>
__forceinline void CHTMLArchive<STREAM>::NextItem( Operation operation /*!< the next type of operation*/ ) {
   _CHECK( (m_uTagState & eNextMask) == 0 );
...
//! end current or all nodes
template<class STREAM>
void CHTMLArchive<STREAM>::EndItem( EndOperation operation /*!< type of end operation*/ )
{
...
template<class STREAM>
void CHTMLArchive<STREAM>::SetHeader( const wchar_t* pszText /*!< header text */ )
{
   _CHECK( m_uState & eWrite );
   _CHECK_S( pszText );
...
template<class STREAM>
void CHTMLArchive<STREAM>::SetAttribute( const wchar_t* pszName, /*!< name for attribute*/ 
...
template<class STREAM>
__forceinline void CHTMLArchive<STREAM>::SetValue( const wchar_t* pszValue ) {
   _SetValue( (const unsigned char*)pszValue, -1 ); 
}

template<class STREAM>
__forceinline void CHTMLArchive<STREAM>::SetBinaryValue( const BYTE* pbValue, int iLength ) {
   _CHECK( iLength >= 0 );
...
template<class STREAM>
void CHTMLArchive<STREAM>::_SetValue( const unsigned char* pValue, int iLength )
{
   _CHECK( m_uState & eWrite );
   _CHECK( m_uTagState & eName );
...
template<class STREAM>
void CHTMLArchive<STREAM>::SetItem( const CHTMLItem& item /*!< item that is set to stream */ )
{
   _CHECK( m_uState & eWrite );
   if( m_uTagState & eNextMask )
   {
      // it has to be a name if child, sibling or attribute has been set
      _CHECK( item.GetType() == CHTMLItem::eTSTR );
   ...
   switch( item.GetType() )
   {
   case CHTMLItem::eTUI1 :
      V.ul = item.GetData().valUChar;
      goto u4;
   case CHTMLItem::eTUI2 :
      V.ul = item.GetData().valUShort;
      goto u4;
   case CHTMLItem::eTUI4 :
...
u4:
   _ultot( V.ul, m_stringBuffer.GetBufferEnd( _LONG_MAX ), 10 );
   m_stringBuffer.ReleaseBufferFromEnd();
   goto exit;
i4:
   _ltot( V.l, m_stringBuffer.GetBufferEnd( _LONG_MAX ), 10 );
   m_stringBuffer.ReleaseBufferFromEnd();
   goto exit;
r8:
#ifndef _UNICODE
...
template<class STREAM>
HRESULT CHTMLArchive<STREAM>::SkipHeader()
{
   wchar_t ch;
   UINT uLength;
   bool bBreak;
...
//! return level
template<class STREAM>
__forceinline unsigned CHTMLArchive<STREAM>::GetSection() {
   return m_stringsStack.GetCount();
}
...
template<class STREAM>
__forceinline int CHTMLArchive<STREAM>::GetName( wchar_t* pszBuffer, int iMaxLength ) const {
...
template<class STREAM>
gd::CString CHTMLArchive<STREAM>::GetParentName() const {
...
template<class STREAM>
__forceinline int CHTMLArchive<STREAM>::GetNextName( wchar_t* pszBuffer, int iMaxLength ) const {
   return _GetName( pszBuffer, iMaxLength, &m_stringNext );
}

template<class STREAM>
int CHTMLArchive<STREAM>::_GetName( wchar_t* pszBuffer, int iMaxLength, const gd::CString* pstringNode ) const
{
...
//! return the node name
template<class STREAM>
__forceinline gd::CString CHTMLArchive<STREAM>::GetName() const {
...
template<class STREAM>
__forceinline gd::CString CHTMLArchive<STREAM>::GetNextName() const {
   return _GetName( &m_stringNext );
}

template<class STREAM>
gd::CString CHTMLArchive<STREAM>::_GetName( const gd::CString* pstringNode ) const
{
   wchar_t* pszPosition;
...
template<class STREAM>
int CHTMLArchive<STREAM>::GetValue( wchar_t* pszBuffer, int iMaxLength ) const
{
   wchar_t* pszPosition;
   wchar_t* pszCheck;
...
template<class STREAM>
gd::CString CHTMLArchive<STREAM>::GetValue() const
{
   wchar_t* pszPosition;
   wchar_t* pszCheck;
...
template<class STREAM>
gd::CString CHTMLArchive<STREAM>::GetTextAfterEndTag() const
{
...
template<class STREAM>
__forceinline int CHTMLArchive<STREAM>::GetBinaryValue( const wchar_t* pszName, BYTE* pBuffer, int iLength ) const {
...
template<class STREAM>
unsigned CHTMLArchive<STREAM>::GetAttributeCount() const {
   unsigned uCount;
...
//! attribute.
template<class STREAM>
__forceinline void CHTMLArchive<STREAM>::ReadAttributeName( const wchar_t* pszNameStart, gd::CString* pstringName ) {
   pstringName->Single();
...
template<class STREAM>
gd::CString CHTMLArchive<STREAM>::GetAttributeName( unsigned uIndex ) const {
   int iPosition;
...
template<class STREAM>
__forceinline int CHTMLArchive<STREAM>::GetAttribute( const wchar_t* pszName, wchar_t* pszBuffer, int iMaxLength ) const {
   return _GetAttribute( true, pszName, (BYTE*)pszBuffer, iMaxLength );
}

template<class STREAM>
gd::CString CHTMLArchive<STREAM>::GetAttribute( const wchar_t* pszName ) const
{
   gd::CString string;
...
template<class STREAM>
int CHTMLArchive<STREAM>::GetAttribute( unsigned uIndex, wchar_t* pszBuffer, int iMaxLength ) const
{
   iPosition = _FindAttribute( uIndex );
   if( iPosition != -1 )
   {
...
template<class STREAM>
gd::CString CHTMLArchive<STREAM>::GetAttribute( unsigned uIndex ) const 
{
   int iPosition;
   gd::CString string;
...
template<class STREAM>
int CHTMLArchive<STREAM>::_GetAttribute( bool bText, const wchar_t* pszName, BYTE* pBuffer, int iMaxLength ) const
{
   _CHECK( iMaxLength > 0 );
...
template<class STREAM>
int CHTMLArchive<STREAM>::_CopyValue( bool bText, const wchar_t* pszPosition, BYTE* pBuffer, int iMaxLength ) const
{
   wchar_t chEnd = 0;
...
template<class STREAM>
int CHTMLArchive<STREAM>::_GetAttributeSize( bool bText, const wchar_t* pszName ) const
{
   int iPosition;
   wchar_t chEnd;
   const wchar_t* pszPosition;
 ...
 */
template<class STREAM>
int CHTMLArchive<STREAM>::_GetAttributeSize( bool bText, unsigned uIndex ) const
{
   int iPosition;
   const wchar_t* pszPosition;
...
template<class STREAM>
int CHTMLArchive<STREAM>::_FindAttribute( unsigned uIndex ) const {
   const wchar_t* pszPosition;
...
template<class STREAM>
int CHTMLArchive<STREAM>::FindAttribute( const wchar_t* pszName ) const
{
   _CHECK_S( pszName );
...
template<class STREAM>
unsigned CHTMLArchive<STREAM>::MakeAttributeIndex( wchar_t const **  ppszIndex, unsigned uCount )
{
   unsigned uIndex;
   const wchar_t* pszPosition;
...
template<class STREAM>
HRESULT CHTMLArchive<STREAM>::GetItem( const wchar_t* pszName,/*!< name of the attribute that value is read from*/
                                      CHTMLItem& item /*! reference to item that gets value */ )
{
   int    iPosition;
   wchar_t  chSave;
   wchar_t* pszEnd;
...
exit:
   ...
   switch( item.GetType() )
   {
   case CHTMLItem::eTUI1 :  item.m_V.valUChar = (unsigned char)V.ul; break;
   case CHTMLItem::eTUI2 :  item.m_V.valUShort = (unsigned short)V.ul; break;
   case CHTMLItem::eTUI4 :  item.m_V.valULong = V.ul; break;
   case CHTMLItem::eTI1 :   item.m_V.valChar = (char)V.l; break;
   case CHTMLItem::eTI2 :   item.m_V.valShort = (short)V.l; break;
   case CHTMLItem::eTI4 :   item.m_V.valLong = V.l; break;
   case CHTMLItem::eTR4 :   item.m_V.valFloat = (float)V.d; break;
   case CHTMLItem::eTR8 :   item.m_V.valDouble = V.d; break;
   case CHTMLItem::eTSTR :  item.m_V.valpsz = pszPosition; break;
...
template<class STREAM>
__forceinline bool CHTMLArchive<STREAM>::ReadNext() {
   return Read();
}
...
template<class STREAM>
bool CHTMLArchive<STREAM>::ReadNext( const wchar_t* pszName ) {
...
//! read next item in XML text
template<class STREAM>
__forceinline bool CHTMLArchive<STREAM>::PreReadNext() {
   m_uState |= ePreRead;
   return Read();
}

/////////////////////////////////////////////////
//! pop nodename from internal data of CHTMLArchive
template<class STREAM>
gd::CString CHTMLArchive<STREAM>::PopNode() {
 ...
 */
template<class STREAM>
bool CHTMLArchive<STREAM>::CompareName( const wchar_t* pszName )
{
...
//! end current node
template<class STREAM>
void CHTMLArchive<STREAM>::EndNode( CHTMLArchive<STREAM>* pThis )
{
...
template<class STREAM>
bool CHTMLArchive<STREAM>::IsEnd() const
{
...
/*---------------------------------------------------------------------------------*/ /**
 * Flush buffer
 \note CHTMLArchive process the XML file in sections. If the XML file is very large then it
 ...
 */
template<class STREAM>
void CHTMLArchive<STREAM>::Flush()
{
   UINT uBytes;
   if( m_uState & eRead )
   {
 ...
 * Read is the main workhorse parsing XML files.<br>
 ...
 CHTMLArchive can read one node ahead to se whats coming. This is often what you need to
 ...
 - Flow
   ...
   -# Read element data.<br>
      ...
      CHTMLArchive uses buffered reading _CompareText function is used to check if CDATA is found.
 ...
 */
template<class STREAM>
bool CHTMLArchive<STREAM>::Read()
{
#define _htmlarchive_ATTRIBUTES 1
#define _htmlarchive_VALUE     0
#define _htmlarchive_COPYVALUE 2

   _CHECK( m_uState & eRead );

   unsigned uSpace = _htmlarchive_ATTRIBUTES;
   wchar_t ch;
   UINT* puState;
   const wchar_t* pszPosition;
   gd::CString* pstring;
   ...
   for(;;)
   {
      while( *m_pszPosition && *m_pszPosition != L'<' )
      {
         ...
         {
            if( m_stringsStack.GetCount() ) m_sError = _htmlarchive_NullCharacterFound;
            return false;
         }
      }
   ...
   if( ch == L'\0' )
   {
      m_sError = _htmlarchive_NullCharacterFound;
      if( m_stringsStack.GetCount() ) m_sError = _htmlarchive_NullCharacterBeforeEndNode;
      return false;
   }
   ...
   if( ch == L'/' )
   {
      ...
      if( *m_pszPosition == 0 )
      {
         m_sError = _htmlarchive_NullCharacterBeforeEndNode;
         return false;
      }
      ...
      {
         m_sError = _htmlarchive_EndingDoesNotMatchParent;
         return false;
...
template<class STREAM>
void CHTMLArchive<STREAM>::_SkipTag()
{
   wchar_t ch;
   ...
   if( *m_pszPosition == _U('-') )
   {
      ...
      if( *m_pszPosition == _U('-') )
      {
         ...
         while( true )
         {
            ...
            {
               m_pszPosition++;
               if( m_pszStop <= m_pszPosition ) return;
	            if( m_pszPosition >= m_pszMax ) Flush();
            }
            _In();                                          
   ...
   while( *m_pszPosition && *m_pszPosition != _U('<' ) )
   {
      ...
      {
         m_sError = _htmlarchive_XMLHasEndedBeforeEndOfTags;
         return;
      }
	   if( m_pszPosition >= m_pszMax ) Flush();
   }
...
template<class STREAM>
bool CHTMLArchive<STREAM>::_CompareText( CHTMLArchive<STREAM>* pThis, wchar_t* pszText, int iLength )
{
   bool bOk;
   int iRead;
...
template<class STREAM>
void CHTMLArchive<STREAM>::Write()
{
...
template<class STREAM>
bool CHTMLArchive<STREAM>::ValidateActiveTag()
{
...
template<class STREAM>
__forceinline gd::CString CHTMLArchive<STREAM>::GetActiveTagText() {
   return m_stringBuffer; }


}; // namespace gd_html_stream

#endif // !defined(AFX_HTMLARCHIVE_H__664C0238_A109_453B_9341_6AF4614AD628__INCLUDED_)
