...
#define _UNICODE
...
#if _MSC_VER >= 1300   // if VC++ version .NET
	#define MultiByteToWideChar(CodePage, dwFlags, lpMultiByteStr, cbMultiByte, lpWideCharStr, cchWideChar) \
		...
		(gds_loc).out( gds_state, \
		lpMultiByteStr, &(lpMultiByteStr[cbMultiByte]), (const char *&) gds_pszNext, \
		lpWideCharStr, &lpWideCharStr[cchWideChar], gds_pwszNext);
#else
	#define MultiByteToWideChar(CodePage, dwFlags, lpMultiByteStr, cbMultiByte, lpWideCharStr, cchWideChar) \
		std::use_facet<codecvt<wchar_t, char, mbstate_t> > \
		(gds_loc,0,true).in( gds_state, \
		lpMultiByteStr, &lpMultiByteStr[cbMultiByte], gds_pszNext, \
		lpWideCharStr, &lpWideCharStr[cchWideChar], gds_pwszNext);
#endif
...
#define _SINGLE _Safe()
   ...
   #define _SETPSZ() m_psz = (wchar_t*)(m_pData+1)
#else
   #define _SETPSZ()
#endif
...
#ifdef WIN32
#ifdef _DEBUG
//   #define CHECK_STRING( string ) _ASSERT( ::IsBadStringPtrW( string, -1 ) == FALSE )
...
wchar_t* CString::GetBufferSetLength(
...
PURPOSE: 
RETURN:  */
CString& CString::FormatAdd( const wchar_t* pszFormat, ... )
{
   _SINGLE;
...
PURPOSE: 
RETURN:  */
void CString::_F( const wchar_t* pszFormat, va_list arguments )
{
   unsigned uLength;
   const wchar_t* psz;
   wchar_t pszBuffer[64];
      ...
      else
      {
         ...
         case L'%' :
            *pszBuffer = L'%';
            ...
            break;
         case L'd' :
            _itow_s( va_arg( arguments, int ), pszBuffer, 64, 10 );
            if( psz != NULL )
            {
               uLength = wcslen( psz ) + 1;
            }
            break;
         case L'g' :
//#ifdef _UNICODE
...
//#endif
            break;
         case L'I' :
            if( wcsncmp( pszFormat, L"I64d", 4 ) == 0 )
            {
               _i64tow_s( va_arg( arguments, __int64 ), pszBuffer, 64, 10 );
               if( psz != NULL )
               {
                  uLength = wcslen( psz ) + 1;
               }
            ...
            break;
         case L's' :
            psz = va_arg( arguments, const wchar_t* );
            if( psz != NULL )
            {
               uLength = wcslen( psz ) + 1;
            }
         ...
         {
            _AllocAdd( uLength );
            memcpy( m_pData->End(), psz, uLength * sizeof(wchar_t) );
...
{
   wchar_t pszDouble[40];
//#ifdef _UNICODE
   {
      char pbszBuffer[40];
      _gcvt_s(pbszBuffer, 40, dValue, 35); 
      ::MultiByteToWideChar( 0, 0, pbszBuffer, strlen( pbszBuffer), pszDouble, 40 );
   }
//#else
...
void CString::Add( char chbCharacter ) 
{
   ...
   m_pData->End()[0] = (wchar_t)chbCharacter;
   m_pData->End()[1] = 0;
   m_pData->m_iLength++;
}
...
void CString::Add( wchar_t wchCharacter )
{
   ...
   m_pData->End()[0] = (wchar_t)wchCharacter;
   m_pData->End()[1] = 0;
   m_pData->m_iLength++;
}
#else
...
//#ifdef _UNICODE
   wchar_t* pwsz = MP_NEW_ARR( wchar_t, iLength); 
...
void CString::Add( const char* pbszText, int iLength )
{
   ...
   iLength = ( iLength <= iTemp ) ? iLength : iTemp;
//#ifdef _UNICODE
   WCHAR* pwsz = MP_NEW_ARR( unsigned short, iLength ); 
   ::MultiByteToWideChar( CP_ACP, 0, pbszAdd, iLength, pwsz, iLength ); 
   memcpy( m_pData->End(), pwsz, iLength * sizeof(unsigned short) );
//#else
//   memcpy( m_pData->End(), pbszAdd, iLength );
...
void CString::Add( const wchar_t* pwszText, int iLength )
{
   ...
   iLength = ( iLength <= iTemp ) ? iLength : iTemp;
//#ifdef _UNICODE
   memcpy( m_pData->End(), pwszAdd, iLength * sizeof(wchar_t) );
//#else
//   char* pbsz = (char*)MP_NEW_ARR( iLength ); 
...
void CString::Add( const CString& s )
{
   ...
   memcpy( m_pData->End(), (LPCWSTR)s, s.GetLength() * sizeof(wchar_t) );
...
\endcode
 */
void CString::Insert( int iPosition, const wchar_t* pszText, int iRepeat )
{
   ...
   else
   {
      ...
      memmove( &pszPosition[iSumLength], pszPosition, (m_pData->GetLength() - iPosition) * sizeof(wchar_t) );
   }

   while( iRepeat != 0 )
   {
      iRepeat--;
      memcpy( pszPosition, pszText, iLength * sizeof(wchar_t) );
      pszPosition += iLength;
   }
 ...
 */
bool CString::CompareStartNoCase( const wchar_t* pszText )
{
   unsigned uLength = m_pData->GetLength();
   unsigned u;
   for( u = 0; u < uLength && pszText[u] != L'\0'; u++ );
   if( u < uLength ) uLength = u;

   return (_wcsnicmp( pszText, m_pData->GetText(), uLength) == 0);
}
 ...
 */
bool CString::CompareEnd( const wchar_t* pszText ) 
{
   unsigned uLength = m_pData->GetLength();
   unsigned u;
   for( u = 0; u < uLength && pszText[u] != L'\0'; u++ );
   if( u < uLength ) uLength = u;
   return (memcmp(m_pData->End() - uLength, pszText, uLength * sizeof(wchar_t) ) == 0 ); 
}
...
bool CString::CompareEndNoCase(
   const wchar_t* pszText //!< text that is compared with
   )
{
   unsigned uLength = m_pData->GetLength();
   unsigned u;
   for( u = 0; u < uLength && pszText[u] != L'\0'; u++ );
   if( u < uLength ) uLength = u;
   return (_wcsnicmp( pszText, m_pData->End() - uLength, uLength) == 0);
}
...
void CString::Trim(
   const wchar_t* pszCharacters, // characters that is going to be removed
...
//      m_pData->SetLength( m_pData->GetLength() - ((m_pData->End() - 1) - pszPosition) );
      m_pData->SetLength( m_pData->GetLength() - ((m_pData->End()) - pszPosition) );
      m_pData->End()[0] = 0;
   }
   else
   {
...
void CString::Cut( int iStart, int iLength )
{
   ...
   {
      memmove( &m_pData->GetText()[iStart], &m_pData->GetText()[iStart + iLength], (m_pData->GetLength() - iLength) * sizeof(wchar_t) );
   }
   else
   {
...
//#ifdef _UNICODE
   _wcslwr_s( m_pData->GetText(), m_pData->GetLength()+1 );
//#else
...
//#ifdef _UNICODE
   _wcsupr_s( m_pData->GetText(),  m_pData->GetLength()+1 );
//#else
 ...
 */
void CString::Replace( const wchar_t* pszOld, const wchar_t* pszNew, int iCount )
{
   CHECK_STRING( pszOld );
   ...
   iOldLength = wcslen( pszOld );
   if( pszNew != NULL ) iNewLength = wcslen( pszNew );
   else iNewLength = 0;
   ...
   while( *pszText != 0 )
   {
      if( *pszText == *pszOld )
      {
         if( wcsncmp( pszText, pszOld, iOldLength ) == 0 )
         {
            if( iDifference != 0 )
            {
               if( iDifference > 0 )
               ...
               memmove( &pszText[iOldLength + iDifference], 
                        &pszText[iOldLength], 
                        (m_pData->GetLength() - (&pszText[iOldLength] - m_pData->GetText())) * sizeof(wchar_t) );
               m_pData->SetLength( m_pData->GetLength() + iDifference );
               m_pData->End()[0] = 0;
            }// end if

            if( pszNew != NULL ) memcpy( pszText, pszNew, iNewLength * sizeof(wchar_t) );
 ...
 * \return String object containing text from start position at specified index with the length
   from there
 */
CString CString::Mid( int iStart, const wchar_t* pszStop ) const
{
//   ASSERT( pszStop != NULL );
 ...
 * \return String object containing text from start position at specified index with the length
   from there
 */
CString CString::Mid( const wchar_t* pszStart, const wchar_t* pszStop ) const
{
   int iStart;
...
int CString::Find(
   const wchar_t* pszFind, // text to search for
   ...
   ) const
{
   ...
   const wchar_t* pszText;
   unsigned uPosition;
   unsigned uLength = wcslen( pszFind );
   ...
   while( uPosition != 0 )
   {
      ...
      {
         if( memcmp( pszFind, &pszText[uPosition], uLength * sizeof( wchar_t ) ) == 0 ) return uPosition;
      }
   }
   ...
   ) const
{
   ...
   while( *pszPosition != L'\0' )
   {
      ...
      {
         if( wcsncmp( pszPosition, pszText, uLength ) == 0 )
         {
            uCount++;
...
{
   int iLength;
   iLength = wcslen( m_pData->GetText() );
   m_pData->SetLength( iLength );
}
...
*/
void CString::Strip( 
   const wchar_t* psz //!< pointer to buffer containing characters that should be removed
   )
{
   _SINGLE;

   unsigned uDecrease = 0;
   unsigned uLength = wcslen( psz );
   wchar_t* pszPosition, *pszStore;
...
//#ifdef _UNICODE
      pData = (_Data*) MP_NEW_ARR( BYTE, iSize * sizeof(wchar_t));
...
#ifdef _UNICODE
      pData = (_Data*) MP_NEW_ARR( BYTE, iLength * sizeof(wchar_t));
      iLength += sizeof(_Data) / 2;
#else
      pData = (_Data*) MP_NEW_ARR( BYTE, iLength);
#endif
      pData->m_iMaxLength = iLength - sizeof( _Data );
      memcpy( pData->GetText(), GetText(), (GetLength() + 1) * sizeof(wchar_t) );
