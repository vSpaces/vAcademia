...
u4:
   _ultow_s( V.ul, m_stringBuffer.GetBufferEnd( _LONG_MAX ), _LONG_MAX, 10 );
   m_stringBuffer.ReleaseBufferFromEnd();
   goto exit;
i4:
   _ltow_s( V.l, m_stringBuffer.GetBufferEnd( _LONG_MAX ), _LONG_MAX, 10 );
   m_stringBuffer.ReleaseBufferFromEnd();
   goto exit;
r8:
//#ifndef _UNICODE
...
//#else
   ...
   _gcvt_s( pbszBuffer, _DOUBLE_MAX, V.d, _DOUBLE_MAX - 2 );
...
void CXMLArchive<STREAM>::Flush()
{
   ...
   if( m_uState & eRead )
   {
      ...
      {
         wcscpy_s( m_pszStart, m_uBufferSize, m_stringInFront );
