...
public:
   ...
   CThreadT(HANDLE hThread = NULL) : m_hThread(hThread), m_bSuspended(false), m_uStackSize(64000)
   {
   }
   virtual ~CThreadT()
   {
   ...
   BOOL CreateViskoe(LPTHREAD_START_ROUTINE pThreadProc, 
               LPVOID pParam = NULL, 
               int iPriority = THREAD_PRIORITY_NORMAL)
   {
      ...
      DWORD dwThreadID;
#if defined(_MT) || defined(_DLL)
      m_hThread = (HANDLE) _beginthreadex(NULL, m_uStackSize, (UINT (WINAPI*)(void*)) pThreadProc, pParam, CREATE_SUSPENDED, (UINT*) &dwThreadID);
#else
      m_hThread = ::CreateThread(NULL, m_uStackSize, pThreadProc, pParam, CREATE_SUSPENDED, &dwThreadID);
#endif
   ...
   void SetStackSize( unsigned int auStackSize)
   {
		m_uStackSize = auStackSize;
   }
   BOOL Release()
   {
...
public:
   ...
   {
      m_bStopped = false;
      if( !CreateViskoe(ThreadProc, (LPVOID) static_cast<T*>(this) ) ) return FALSE;
      return TRUE;
   }
   virtual void Stop()
   {
...
public:
   ...
   {
      _ASSERTE(pstrName==NULL || !::IsBadStringPtr(pstrName, (UINT_PTR)-1));
   ...
   {
      _ASSERTE(!::IsBadStringPtr(pstrName,(UINT_PTR)-1));
      _ASSERTE(m_hEvent==INVALID_HANDLE_VALUE);
