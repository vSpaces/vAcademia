...
public:
	...
	{
		if (InlineIsEqualGUID((REFGUID)riid, (REFGUID)IID_IConnectionPoint) || InlineIsEqualUnknown(riid))
		{
			if (ppvObject == NULL)
				return E_POINTER;
			*ppvObject = this;
	...
	IEnumConnections** ppEnum)
{
	...
	*ppEnum = NULL;
	...
	// don't copy the pData, but transfer ownership to it
	...
	int nConnectionIndex)
{
	...
	if (dwGITCookie != NULL)
	{
		IID iid;
		GetConnectionInterface(&iid);
		// Если здесь возвращается hr!=S_OK, то значит в вызывающем потоке
		// нет вызова CoInitialize(NULL)
		HRESULT hr = m_pGIT->GetInterfaceFromGlobal(
