...
namespace audiere {
...
#if AUDIERE_BUILD_H
	m_mutex.lock();
	if ( m_frames_left_in_chunk >= m_data_length || m_data_length == 0)
	{
		m_mutex.unlock();
		return 0;
	}
	const int frames_to_read = std::min(frame_count, m_data_length - m_frames_left_in_chunk);
	m_mutex.unlock();
#else
  ...
  WAVInputStream::setPosition(int position) {
	  ...
	  int frame_size = m_channel_count * GetSampleSize(m_sample_format);
	  m_mutex.lock();
	  m_frames_left_in_chunk = position;
	  m_mutex.unlock();
	  if ( m_frames_left_in_chunk < 0 || m_frames_left_in_chunk > m_data_chunk_length)
		  assert( false);
...
#if AUDIERE_BUILD_H
  int
  WAVInputStream::updateFileInfo(int loaded_bytes, bool bLoaded){
    int ibytes = loaded_bytes - m_file->getTotalReadBytes();
    if ( !bLoaded && ibytes < 131072)
      return -1;
	int frame_size = m_channel_count * GetSampleSize(m_sample_format);
	m_mutex.lock();
	if ( !bLoaded)
		m_data_length = m_file->getTotalReadBytes() / frame_size;
	else
		m_data_length = m_data_chunk_length;	
	m_mutex.unlock();
    return 0;
  }
#endif
