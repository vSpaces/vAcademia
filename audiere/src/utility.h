...
namespace audiere {
  ...
  public:
	...
	typedef MP_MAP<MP_STRING, MP_STRING>::const_iterator m_valuesIter;
  private:    
	MP_MAP<MP_STRING, MP_STRING> m_values;	
  };
  ...
  public:
    QueueBuffer() {
	  ...
	  m_buffer = MP_NEW_ARR( unsigned char, m_capacity);
    }
    ...
    void write(const void* buffer, int size) {
      ...
      if (need_realloc) {		  
        //m_buffer = (u8*)realloc(m_buffer, m_capacity);
		  m_buffer = MP_REALLOC_ARR( u8, m_buffer, m_capacity);

		  /*u8* new_buffer = MP_NEW_ARR( u8, m_capacity/sizeof(u8));
