...
#define AUDIERE_BUILD_F 1
#define AUDIERE_BUILD_H 1
#define AUDIERE_RELEASE_H 1
#define AUDIERE_RELEASE_O 1
#define AUDIERE_RELEASE_O_FIX 1
#define AUDIERE_RELEASE_P 1
#define AUDIERE_RELEASE_P_FIX 1
...
namespace audiere {
...
#if AUDIERE_BUILD_H
	/**
	* Get duplicate of the file.
	*
	* @return  current position
	*/
	ADR_METHOD(File*) duplicate() = 0;
	ADR_METHOD(int) readNextNewDataFromFile(void* buffer, int size) = 0;
	ADR_METHOD(int) getTotalReadBytes(){ return 0;};
#endif
  };
  ...
  public:
...
#if AUDIERE_BUILD_H
	/**
	* @return  size of file in the stream, or 0 if the file stream is not
	*          fully loaded (is not support) 
	*/
	ADR_METHOD(int) getSize() = 0;	
#endif
	...
	*/
	ADR_METHOD(int) getHeaderSize() = 0;	
#endif
...
#if AUDIERE_BUILD_H
	/**
	* Update information of loading file
	*
	* @param loaded_bytes  number of bytes loaded on local machine	
	*
	* @return  code of error (> -1 is no error)
	*/
	ADR_METHOD(int) updateFileInfo(int loaded_bytes, bool bLoaded) = 0;

	/**
	* Returns the type of file
	*
	* @return code of file type (FileFormat)
	*/
	ADR_METHOD(FileFormat) getFileType() = 0;
#endif
  ...
  public:
...
#if AUDIERE_BUILD_H
	/**
	* Continue playback of the output stream from last position of source.  If the stream is already
	* playing, this does nothing.
	*/
	ADR_METHOD(void) resume_play() = 0;
#endif
	...
	/**
	* Stop playback of the output stream and wait updating.  If the stream is already
	* stopped, this does nothing.
	*/
	ADR_METHOD(void) stop_and_wait() = 0;
	ADR_METHOD(void) clear( bool bInternal) = 0;
	ADR_METHOD(void) createBuffer() = 0;
#endif
	...
	* @return  frequency
	*/
	ADR_METHOD(int) getFrequency() = 0;

#if AUDIERE_BUILD_H
	/**
	* Update information of file
	*
	* @param loaded_bytes  number of loaded bytes of file
	*
	* @return  error code (> -1 is no error)
	*/
	ADR_METHOD(int) updateFileInfo(int loaded_bytes, bool bLoaded) = 0;
#endif
	...
	/**
	* Set link on soundPtr
	*
	* @return  error code (> -1 is no error)
	*/
	ADR_METHOD(void) setSoundPtrItem(ISoundPtrBase *pSoundPtrBase) = 0;
#endif
  };
...
#if AUDIERE_RELEASE_P
	ADR_METHOD(bool) GetSoundOutputDevices(std::wstring &soundDevices) = 0;
	ADR_METHOD(bool) IsDeviceChangeNeed( std::wstring &sDeviceName) = 0;
	ADR_METHOD(bool) ReplaceDevice( std::wstring &sDeviceName) = 0;
	ADR_METHOD(void) Lock() = 0;
	ADR_METHOD(void) UnLock() = 0;
#endif
  ...
  namespace hidden {
	...
	ADR_FUNCTION(bool) AdrReplaceDevice(
    ...
    char delim)
  {
    ...
    while (*in) {
      ...
      if (next) {        
		out.push_back(MAKE_MP_STRING(std::string(in, next)));
      } else {        
		out.push_back(MAKE_MP_STRING(in));
      }
  ...
  struct FileFormatDesc {
    FileFormatDesc():MP_STRING_INIT(description),MP_VECTOR_INIT(extensions){}
    /// Short description of format, such as "MP3 Files" or "Mod Files"    
	MP_STRING description;

    /// List of support extensions, such as {"mod", "it", "xm"}    
	MP_VECTOR<std::string> extensions;
  };
  ...
  struct AudioDeviceDesc {
	AudioDeviceDesc():MP_STRING_INIT(name),MP_STRING_INIT(description){}
  ...
  inline bool ReplaceDevice(
	  const AudioDevicePtr& device,
	  const std::wstring sDeviceName)
  {
	  return hidden::AdrReplaceDevice(device, sDeviceName);
  }
}
