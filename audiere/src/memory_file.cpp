...
namespace audiere {
  ...
  ADR_EXPORT(File*) AdrCreateMemoryFile(const void* buffer, int size) {
	...
	MP_NEW_P2( pMemoryFile, MemoryFile, buffer, size);
	return pMemoryFile;
  }
  ...
  MemoryFile::MemoryFile(const void* buffer, int size) {
    ...
    m_size = size;    
	m_buffer = MP_NEW_ARR(u8, m_capacity);
  ...
  File* ADR_CALL MemoryFile::duplicate() {	  
	  MP_NEW_P2(memoryFile, MemoryFile, m_buffer, m_size);
	  //memoryFile->m_capacity = m_capacity;
	  memoryFile->m_position = m_position;
	  return memoryFile;
  }

  int ADR_CALL MemoryFile::readNextNewDataFromFile(void* buffer, int size) {
  ...
  void MemoryFile::ensureSize(int min_size) {
    ...
    if (realloc_needed) {      
	  u8* new_buffer = MP_NEW_ARR(u8, m_capacity);
