...
namespace audiere {
  ...
  public:
    MultipleSoundEffect(AudioDevice* device, SampleBuffer* sb):MP_VECTOR_INIT(m_streams) {
      m_device = device;
      m_buffer = sb;
  ...
  private:
	...
	MP_VECTOR<OutputStreamPtr> m_streams;
    ...
    SoundEffectType type)
  {
    ...
    switch (type) {
      case SINGLE: {
		...
		if ( os)
		{
			MP_NEW_P( pSingleSoundEffect, SingleSoundEffect, os);
			return pSingleSoundEffect;
		}
		return 0;
      }
      ...
      case MULTIPLE: {
		...
		if ( sb)
		{
			MP_NEW_P2( pMultipleSoundEffect, MultipleSoundEffect, device, sb);
			return pMultipleSoundEffect;
		}
		return 0;
      }
