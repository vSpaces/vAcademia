...
namespace audiere {
...
#if AUDIERE_BUILD_H
  void 
  DSOutputBuffer::resume_play(){	
	ADR_GUARD("DSOutputBuffer::resume_play");
	play();	
  }
#endif
  ...
  void
  DSOutputBuffer::stop_and_wait() {
	ADR_GUARD("DSOutputBuffer::stop_and_wait");	
	stop();	
  }
#endif
  ...
  int
  DSOutputBuffer::getFrequency() {    
    return m_base_frequency;
  }

#if AUDIERE_BUILD_H
  int
  DSOutputBuffer::updateFileInfo(int loaded_bytes, bool bLoaded) { 
	return 0;
  }
#endif
