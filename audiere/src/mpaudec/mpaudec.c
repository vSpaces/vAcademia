...
int mpaudec_init(MPAuDecContext * mpctx)
{
	...
	//mpctx->priv_data = MP_NEW( MPADecodeContext);
    if (mpctx->priv_data == NULL)
        return -1;
    s = (MPADecodeContext *) mpctx->priv_data;
    ...
    if (!init && !mpctx->parse_only) {
        ...
        for(i=1;i<16;i++) {
            ...
            code_table = (uint8_t *) calloc(n, 1);
			//code_table = (uint8_t *) MP_NEW_ARR( unsigned char, n);
            j = 0;
            for(x=0;x<xsize;x++) {
                for(y=0;y<xsize;y++)
                    code_table[j++] = (x << 4) | y;
...
void mpaudec_clear(MPAuDecContext *mpctx)
{
	...
	/*MPADecodeContext *p = (MPADecodeContext*)mpctx->priv_data;
	MP_DELETE( p);
	mpctx->priv_data = NULL;*/
    memset(mpctx, 0, sizeof(MPAuDecContext));
}
