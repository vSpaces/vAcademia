...
unsigned int show_bits(const GetBitContext *s, int n)
{
	...
	for (i = s->index; i < s->index + n && i < s->size_in_bits; i++) {
#else
...
static int alloc_table(VLC *vlc, int size)
{
    ...
    if (vlc->table_size > vlc->table_allocated) {
		...
		/*int len =  vlc->table_allocated * 2; // * sizeof(VLC_TYPE);
		void *p = (void*)MP_REALLOC_ARR( VLC_TYPE, vlc->table, len);	
		vlc->table = (int16_t (*)[2])p;*/
        if (!vlc->table)
            return -1;
    }
    return index;
