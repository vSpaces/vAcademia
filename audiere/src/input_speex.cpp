...
namespace audiere {
  ...
  SpeexInputStream::initialize(FilePtr file) {
    MP_NEW_P( pFileReader, FileReader, file);
#if defined(_MSC_VER) && (_MSC_VER <= 1200)    
	//m_reader = std::auto_ptr<speexfile::Reader>(pFileReader);
	m_reader = pFileReader;
#else 
	if( m_reader != NULL)
	{
		FileReader *pFileReaderTmp = (FileReader *)m_reader;
		MP_DELETE( pFileReaderTmp);
		m_reader = NULL;
	}
	m_reader = pFileReader;
	//m_reader.reset(pFileReader);
#endif    
	MP_NEW_V(m_speexfile, speexfile::speexfile, m_reader);
