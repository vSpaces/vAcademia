#ifndef INPUT_LAME_MP3_H
#define INPUT_LAME_MP3_H
...
#define MP3LAME_FRAME_SIZE 1152

namespace audiere {
	
	struct TLameFrameSize
	{
		int frame_size;
		int total_size;
	};

	class MP3LameInputStream : public BasicSource {
	public:
		MP3LameInputStream();
		...
		FileFormat ADR_CALL getFileType(){ return FF_LAME_MP3;};
#endif
		...
		*/
		int	 ADR_CALL getHeaderSize(){ return m_header_size;};	
#endif
	...
	private:
		...
		void readID3v1Tags();
		void readID3v2Tags();
		bool decodeFrame(bool parse_only);
		
		int decodeFirstFrame();
		int lame_decode_fromfile(short pcm_l[MP3LAME_FRAME_SIZE], short pcm_r[MP3LAME_FRAME_SIZE], int &len, bool parse_only, bool bInit = false);
		int get_audio_common(short buffer[2][MP3LAME_FRAME_SIZE], int &len, bool parse_only, bool bInit = false);
#if AUDIERE_BUILD_H
		bool decodeNextFrame(bool parse_only, bool bInit = false);
#endif
		...
		int m_position;
#if AUDIERE_BUILD_F		
		MP_VECTOR<TLameFrameSize *> m_frame_sizes;
#else
