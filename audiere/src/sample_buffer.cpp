...
namespace audiere {
  ...
  public:
      ...
      int channel_count, int sample_rate, SampleFormat sample_format)
    {
	  ...
	  m_samples = MP_NEW_ARR(u8, buffer_size);
      if (samples) {
    ...
    SampleSource* ADR_CALL openStream() {      
	  MP_NEW_P( pBufferStream, BufferStream, this);
	  return pBufferStream;
    }
  ...
  ADR_EXPORT(SampleBuffer*) AdrCreateSampleBuffer(
    ...
    SampleFormat sample_format)
  {    
	MP_NEW_P5(pSampleBufferImpl, SampleBufferImpl, samples, frame_count,
				channel_count, sample_rate, sample_format);
	return pSampleBufferImpl;
  }
    ...
    SampleSource* source)
  {
	...
	u8* buffer = MP_NEW_ARR(u8, stream_length_bytes);
