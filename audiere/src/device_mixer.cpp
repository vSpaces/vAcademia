...
namespace audiere {
  ...
  MixerDevice::MixerDevice(int rate):MP_LIST_INIT(m_streams) {
    m_rate = rate;
  }
  ...
  MixerDevice::openStream(SampleSource* source) {    
	if ( source)
	{
       MP_NEW_P3( mixStream, MixerStream, this, source, m_rate);
	   return mixStream;
	}
	return 0;
  }
...
//    ADR_LOG("done locking mixer device");
  ...
  {
    m_device     = device;    
	MP_NEW_V2( m_source, Resampler, source, rate);
    m_last_l     = 0;
    m_last_r     = 0;
    m_is_playing = false;
    m_volume     = 255;
