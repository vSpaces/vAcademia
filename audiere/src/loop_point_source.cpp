...
namespace audiere {
  ...
  public:
    LoopPointSourceImpl(SampleSource* source):MP_VECTOR_INIT(m_loop_points) {
      source->reset();
      m_source = source;
...
#if AUDIERE_BUILD_H
	FileFormat ADR_CALL getFileType(){ return m_source->getFileType();};
#endif
	...
	*/
	...
	int  ADR_CALL updateFileInfo(int loaded_bytes, bool bLoaded){return 0;};
#endif
  ...
  private:
	...
	MP_VECTOR<LoopPoint> m_loop_points;
  };
    ...
    SampleSource* source)
  {
	...
	MP_NEW_P( pLoopPointSource, LoopPointSourceImpl, source);
	return pLoopPointSource;
  }
