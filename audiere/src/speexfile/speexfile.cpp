            ...
            while ( ogg_stream_packetout (&decoder->os, &decoder->op) == 1 ) {
                if ( decoder->op.b_o_s || (current_serial != ogg_page_serialno (&decoder->og)) ) {
                    ...
                    if ( !seekable ) {
						...
						stream = (speexstream_t **)MP_REALLOC_ARR( pspeexstream_t, stream, (streamcount+1) * sizeof (speexstream_t*));
                        if ( !stream ) {
						...
						stream[streamcount] =  MP_NEW_Z( speexstream_t);
						...
						stream[streamcount]->seekinfo =  (speexseekinfo_t **)MP_NEW_Z( pspeexseekinfo_t);
...
int speexfile::readtags ( char *tagdata, long size )
{
	...
	stream[streamcount-1]->tags = MP_NEW( pspeextags);
    ...
    */
    ...
    if ( nb_fields > 0 ) {
		...
		stream[streamcount-1]->tags = (speextags **)MP_REALLOC_ARR( pspeextags, stream[streamcount-1]->tags, (nb_fields+1) * sizeof (speextags*));
        if ( stream[streamcount-1]->tags == NULL ) {
            strcpy ( speex_last_error, "Memory allocation failed" );
            return -1;
        }
        ...
        for ( int i = 0; i < nb_fields; i++ ) {
			...
			stream[streamcount-1]->tags[stream[streamcount-1]->tagcount] = MP_NEW_Z( speextags);
            ...
            if ( item ) {
                //stream[streamcount-1]->tags[stream[streamcount-1]->tagcount]->item = (char *)malloc ( ilen+1 );
				stream[streamcount-1]->tags[stream[streamcount-1]->tagcount]->item = MP_NEW_ARR(char, ilen+1);
                if ( stream[streamcount-1]->tags[stream[streamcount-1]->tagcount]->item == NULL ) {
                    strcpy ( speex_last_error, "Memory allocation failed" );
                    return -1;
                }
            ...
            if ( value ) {				
                //stream[streamcount-1]->tags[stream[streamcount-1]->tagcount]->value = (char *)malloc ( vlen+1 );
				stream[streamcount-1]->tags[stream[streamcount-1]->tagcount]->value = MP_NEW_ARR(char, vlen+1);
                if ( stream[streamcount-1]->tags[stream[streamcount-1]->tagcount]->value == NULL ) {
                    strcpy ( speex_last_error, "Memory allocation failed" );
                    return -1;
                }
        ...
        while ( ogg_sync_pageout (&oy, &og) == 1 ) {
            ...
            while ( ogg_stream_packetout (&os, &op) == 1 ) {
                if ( op.b_o_s ) {
                    ...
                    void *stream_b = (void *)stream;					
                    //stream = (speexstream_t **)realloc ( stream, (streamcount+1) * sizeof (speexstream_t*) );
					stream = MP_REALLOC_ARR( pspeexstream_t, stream, (streamcount+1) * sizeof (speexstream_t*) );
                    if ( !stream ) {
                        stream = (speexstream_t **)stream_b;
                        strcpy ( speex_last_error, "Memory allocation failed" );
                        return -1;
                    ...
                    //stream[streamcount]->seekinfo = (speexseekinfo_t **)calloc ( 1, sizeof (speexseekinfo_t*) );
					 stream[streamcount]->seekinfo = MP_NEW_Z( pspeexseekinfo_t);
                    if ( !stream[streamcount]->seekinfo ) {
                        strcpy ( speex_last_error, "Memory allocation failed" );
                        return -1;
                    }
                ...
                if ( op.packetno == 1 ) {   // extract tags
					...
					stream[spos]->seekinfo = (speexseekinfo_t **)MP_REALLOC_ARR( pspeexseekinfo_t, stream[spos]->seekinfo, (stream[spos]->sicount+1) * sizeof (speexseekinfo_t*));
					...
					stream[spos]->seekinfo[stream[spos]->sicount] = MP_NEW(speexseekinfo_t);
...
void *speexfile::header_to_decoder ( SpeexHeader *header, int enh_enabled, int *frame_size, int *rate, int *nframes, int forceMode, int *channels, SpeexStereoState *stereo )
{
    ...
    if ( forceMode != -1 )
        modeID = forceMode;
    mode = (SpeexMode *) speex_lib_get_mode(modeID);//speex_mode_list[modeID];
