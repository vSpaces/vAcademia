...
typedef struct CAL3D_API _submesh_face_point{
	...
	int			max_mesh_id;
#endif
	std::vector<ctrlp_infl>	influences;
...
typedef struct CAL3D_API _control_point
{
	...
	std::vector<submesh_face_point>	points;		// массив вершин данной контрольной точки
	std::vector<NDef>				nbrs;		// массив соседних контрольных вершин
...
typedef struct _FACEExpressionsPack
{
	...
	// Variables
	std::vector<FACEExpression*>	pexpressions;
	int	count;
} FACEExpressionsPack;
...
public:
	...
	*/
	...
	std::vector<fap_on_face>	cur_faps;

	std::vector<submesh_face_point>	infl_array;
	int								lastindex;

	std::vector<submesh_face_point>	infl_points;	// передвигаемые точки
	LPVOID							pBone;

	std::vector<submesh_face_point>	control_points;
	bool	is_control_point(int mid, int smid, int id);

	// Runtime
	std::vector<FACEExpression*>		expressions;
	std::vector<control_point*>			cur_cpoints;
