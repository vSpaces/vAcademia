...
//sprintf( buf, "gr=%d, id=%d\n", group_id, in_group_id);OutputDebugString( buf);
	...
	{
		n_points[group_id][in_group_id] = (control_point*)new control_point(group_id, in_group_id);
		cp_num++;
	}
...
void CMpeg4FDP::calculate_FAPU(CalCoreMesh *pmesh)
{
	...
	*/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	int pointsSize = rootPoint->points.size();
...
void	CMpeg4FDP::savelog()
{
	...
	for( int ig=0; ig<MAX_CONTROL_GROUP; ig++)
	{
		sprintf((char*)buf, " GROUP %d", ig);
		log.writeln(buf);
		for( int in=0; in<MAX_CONTROL_POINTS_INGROUP; in++)
		{
			if( !n_points[ig][in])	continue;
			sprintf((char*)buf, "  SUB_GROUP %d", in);
			log.writeln(buf);
			control_point* point = n_points[ig][in];
			for(int i=0; i<point->points.size(); i++){
				submesh_face_point	sp=point->points[i];
				sprintf((char*)buf, "   submesh_id=%d, id=%d, max_id=%d", sp.submesh_id, sp.id, sp.max_mesh_id+1);
				log.writeln(buf);
			}
		}
	}
...
void CMpeg4FDP::clear()
{
	...
	for( int ig=0; ig<MAX_CONTROL_GROUP; ig++)
		for( int in=0; in<MAX_CONTROL_POINTS_INGROUP; in++)
			if( n_points[ig][in])	delete n_points[ig][in];

	memset( &n_points, 0, 4*MAX_CONTROL_GROUP*MAX_CONTROL_POINTS_INGROUP);
	memset( &IRISD0, 0, 6*sizeof(float));
}
...
void CMpeg4FDP::apply_fap(FACEExpression *exp)
{
	...
	{
		for( int i=0; i<exp->expression->faps.size(); i++)
		{
...
#ifdef DEBUG
			if( (cp_desc.group_id==8 && cp_desc.id==2))
				int lerskg=0;
#endif
...
void FACEExpression::init(CMpeg4FDP* face)
{
	...
	{
		for( int i=0; i<expression->faps.size(); i++)
		{
...
bool _FACEExpression::update(float deltaTime)
{
  ...
  // handle IN phase
  if(state == CalAnimation::STATE_IN)
  {
  ...
  // hanbdle STEADY
  if(state == CalAnimation::STATE_STEADY)
  {
	weight = destWeight;
  }
...
FACEExpression* CMpeg4FDP::get_expression(FACEExpression *fe)
{
	for( int i=0; i<expressions.size(); i++)
	{
		...
		if( !fe->name.IsEmpty() && expressions[i]->name == fe->name)
...
bool CMpeg4FDP::initialize(CalModel *pmodel)
{
	...
	int mc, sc;
	CalRenderer* rend = pmodel->getRenderer();
	...
	for( int ip=0; ip<cp->points.size(); ip++)
	{
		...
		if( vectorSubmesh.size() <= sfp->submesh_id)
		{
			sfp = NULL;
			continue;
		}
		...
		for (int i=0; i<inflSize; i++)
		{
			...
			if( influence.boneId < vectorBone.size())
			{
				CalBone* pTempBone = vectorBone[influence.boneId];
				Cal3DComString	sBoneName(pTempBone->getCoreBone()->getName());
...
#ifdef DEBUG
		if( (bcp.group_id==8 && bcp.id==2) || 
			(bcp.group_id==2 && bcp.id==3))
			int lerskg=0;
#endif
