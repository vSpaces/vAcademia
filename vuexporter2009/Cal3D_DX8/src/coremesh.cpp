...
void CalCoreMesh::createShadowData()
{
  ...
  */
	...
	if( m_pShadowData)
		delete m_pShadowData;
	m_pShadowData = new CalShadowData();
	...
	for(iteratorCoreSubmesh = m_vectorCoreSubmesh.begin(); iteratorCoreSubmesh != m_vectorCoreSubmesh.end(); ++iteratorCoreSubmesh)
	{
		...
		for(eit = m_subvectorEdge.begin(); eit != m_subvectorEdge.end(); ++eit, eid++)
		{
			if( eit->if2 == 0xFFFF)
			{
				...
				int	vcount2=vcount+(*iteratorCoreSubmesh)->getVertexCount(),
					fcount2=fcount+(*iteratorCoreSubmesh)->getFaceCount(),
					eCount2 = eCount + (*iteratorCoreSubmesh)->getVectorEdge().size();
				bool freeNeibEdge=false;
				std::vector<CalCoreSubmesh::Vertex>& m_vectorVertex = (*iteratorCoreSubmesh)->getVectorVertex();
//CalVector	v1 = m_vectorVertex[eit->iv1].position;
//CalVector	v2 = m_vectorVertex[eit->iv2].position;
				...
				{
					std::vector<CalCoreSubmesh::Vertex>& m_vectorVertex2 = (*iteratorCoreSubmesh2)->getVectorVertex();
...
//CalVector	_v3 = verts2[faces2[eit2->if1].vertexId[2]].position;
					...
					vcount2 += (*iteratorCoreSubmesh2)->getVertexCount();
					fcount2 += (*iteratorCoreSubmesh2)->getFaceCount();
					eCount2 += (*iteratorCoreSubmesh2)->getVectorEdge().size();
				}
				if( !freeNeibEdge)
				{
					CalCoreSubmesh::Edge	edge = *eit;
		...
		vcount += (*iteratorCoreSubmesh)->getVertexCount();
		fcount += (*iteratorCoreSubmesh)->getFaceCount();
...
CalCoreMesh*	CalCoreMesh::clone()
{
	...
	for (int i=0; i<m_vectorCoreSubmesh.size(); i++)
