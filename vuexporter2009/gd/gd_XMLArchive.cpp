...
inline int _stringstrncmp( const TCHAR* psz1, const TCHAR* psz2, size_t count )
{
...
int CopyXMLText( const TCHAR* pszIn, TCHAR* pszTo, UINT uMaxLength, TCHAR chEnd )
{
   TCHAR ch;
   UINT uAdd;
   UINT uIndex;
   ...
   while( *pszIn != chEnd && uIndex < uMaxLength )
   {
      ...
      else
      {
         ...
         uAdd = 0;
         /*
         if( _stringstrncmp( pszIn, _T("nbsp;"), 4 ) == 0 )
         {
            ch = _T(' ');
            uAdd = 4;
         }
         else 
         */if( _stringstrncmp( pszIn, _T("gt;"), 2 ) == 0 )
         {
            ch = _T('>');
            uAdd = 2;
         }
         else if( _stringstrncmp( pszIn, _T("lt;"), 2 ) == 0 )
         {
            ch = _T('<');
            uAdd = 2;
         }
         else if( _stringstrncmp( pszIn, _T("amp;"), 3 ) == 0 )
         {
            ch = _T('&');
            uAdd = 3;
         }
         else if( _stringstrncmp( pszIn, _T("quot;"), 4 ) == 0 )
         {
            ch = _T('"');
            uAdd = 4;
         }
...
void CopyXMLText( const TCHAR* pszIn, gd::CString* pstringText, TCHAR chEnd )
{
   TCHAR ch;
   UINT uAdd;
...
UINT XMLToTextLength( const TCHAR* pszIn, TCHAR chEnd )
{
   UINT uLength = 0;
...
bool HasXML( const TCHAR* pszXML )
{
   while( *pszXML != 0 )
   {
...
void TextToXML( const TCHAR* pszIn, gd::CString* pstringXML )
{
   pstringXML->Single();
...
gd::CString TextToXML( const TCHAR* pszIn )
{
   gd::CString string;
   string.Single();
   ...
   while( *pszIn != 0 )
   {
      ...
      else
      {
         if( pszWatchFor[*pszIn] & _XML_ENTITY )
         {
            ...
            {
            //case _T(' ') : *pstringXML += _T("&nbsp;"); break;
            case _T('<') : string.FastAdd( _T("&lt;") ); break;
            case _T('>') : string.FastAdd( _T("&gt;") ); break;
            case _T('&') : string.FastAdd( _T("&amp;") ); break;
            //case _T('\'') : *pstringXML += _T("&apos;"); break;
            case _T('"') : string.FastAdd( _T("&quot;") ); break;
...
UINT TextToXMLLength( const TCHAR* pszIn )
{
   UINT uLength = 0;
   ...
   while( *pszIn != 0 )
   {
      ...
      else
      {
         if( pszWatchFor[*pszIn] & _XML_ENTITY )
         {
            ...
            {
            //case _T(' ') : *pstringXML += _T("&nbsp;"); break;
            case _T('<') : uLength += 4; break;
            case _T('>') : uLength += 4; break;
            case _T('&') : uLength += 5; break;
            //case _T('\'') : *pstringXML += _T("&apos;"); break;
            case _T('"') : uLength += 6; break;
            }
         }
         else
         {
...
void TextToHTML( const TCHAR* pszIn, gd::CString* pstringXML )
{
   TCHAR chCharacter;                            
   TCHAR chConvert;
   ...
   while( *pszIn != 0 )
   {
      if( ((unsigned)*pszIn > _T(' ')) && ((unsigned)*pszIn <= _T('z')) && !(pszWatchFor[*pszIn] & _XML_ENTITY) )
      {
         pstringXML->FastAdd( *pszIn );
         pszIn++;
         continue;
      ...
      else
      {
         ...
         {
            pstringXML->FastAdd( _T("&#x") );

            chConvert = (unsigned)*pszIn;
#ifdef _UNICODE
            if( chConvert > 0xff )
            {
               chCharacter = (TCHAR)(chConvert >> 12);                             
               chCharacter += (chCharacter > 9) ? L'7' : L'0';
               pstringXML->FastAdd( chCharacter );
               chCharacter = (TCHAR)(chConvert >> 8) & 0x000f;                             
               chCharacter += (chCharacter > 9) ? L'7' : L'0';
               pstringXML->FastAdd( chCharacter );
               chConvert &= 0x00ff;
            }
#endif
            chCharacter = (TCHAR)((unsigned char)chConvert >> 4);                             
            chCharacter += (chCharacter > 9) ? _T('7') : _T('0');
            pstringXML->FastAdd( chCharacter );
            chCharacter = (TCHAR)(chConvert & 0x0f);
            chCharacter += (chCharacter > 9) ? _T('7') : _T('0');
            pstringXML->FastAdd( chCharacter );
...
void ReplaceXML( TCHAR* pszXML, TCHAR chReplace )
{
   while( *pszXML != NULL )
   {
...
void TextToXMLEx( const TCHAR* pszIn, Texts* pTexts, gd::CString* pstringXML, UINT uFlags )
{
   ...
   while( *pszIn != 0 )
   {
      ...
      if( uFlags & eWWW )
      {
         ...
         {
            if( _tcsnicmp( pszIn, _T("www."), 4 ) == 0 )
            {
               if( pszIn[4] > ' ' ) bFound = true;
            }
            else if( _tcsnicmp( pszIn, _T("http://"), 7 ) == 0 )
            {
      ...
      if( uCount && ((unsigned)*pszIn <= pTexts->uMaxCode) )
      {
         for( uCounter = 0; uCounter < uCount; uCounter++ )
         {
            if( pTextText[uCounter].pszFrom[0] == *pszIn )
            {
               ...
               if( _tcsncmp( pszText, pszIn, _tcslen( pszText ) ) == 0 )
               {
                  pszIn += _tcslen( pszText );
...
gd::CString XMLToText( const TCHAR* pszIn /*!< string that should be converted */)
{
   struct Convert
   {
      TCHAR* pszLookFor;
   ...
   while( *pszIn != 0 )
   {
      ...
      else
      {
         ...
         {
            if( _tcsncmp( pC->pszLookFor, &pszIn[1], pC->lLength ) == 0 )
            {
...
void XMLToText( const TCHAR* pszIn, gd::CString* pstringXML )
{
   struct Convert
   {
      TCHAR* pszLookFor;
...
void BinToHex( const BYTE* pbValue, TCHAR* pszHex, UINT uLength )
{                                                  
   BYTE    bTemp;                                  
   TCHAR   chCharacter;                            
   UINT    uCounter;
   ...
   for( uCounter = 0; uCounter < uLength; uCounter++ )
   {
      ...
      chCharacter = (TCHAR)(bTemp >> 4);                             
      chCharacter += (chCharacter > 9) ? _T('7') : _T('0');
      *pszHex = chCharacter;
      pszHex++;

      bTemp = pbValue[uCounter];
      chCharacter = (TCHAR)(bTemp & 0x0f);
      chCharacter += (chCharacter > 9) ? _T('7') : _T('0');
      *pszHex = chCharacter;
      pszHex++;
   }
...
int HexToBin( const TCHAR* pszHex, BYTE* pbValue, UINT uLength, TCHAR chEnd )
{                                                  
   BYTE    bTemp;//, bTemp2;       
   TCHAR   chCharacter;                            
   UINT    uCounter;
...
UINT HexToBinLength( const TCHAR* pszHex, TCHAR chEnd )
{                                                  
   UINT uCounter = 0;
