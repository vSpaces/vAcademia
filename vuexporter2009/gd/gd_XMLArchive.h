...
extern void CopyXMLText( const TCHAR* pszIn, gd::CString* pstringText, TCHAR chEnd = 0 );
//! copy xml text to text buffer (converts XML tags to normal characters)
extern int  CopyXMLText( const TCHAR* pszIn, TCHAR* pszOut, UINT uMaxLength, TCHAR chEnd = 0 );
//! calculate lenght for xml formated text in normal character format
extern UINT XMLToTextLength( const TCHAR* pszIn, TCHAR chEnd = 0 );
//! check if string has any character that need to be converted to xml entity
extern bool HasXML( const TCHAR* pszXML );
//! check if string has any character that need to be converted to xml entity
extern bool HasHTML( const TCHAR* pszText );
//! convert text to XML string
extern void TextToXML( const TCHAR* pszIn, gd::CString* pstringXML );
//! return text converterd to xml as string object
extern gd::CString TextToXML( const TCHAR* pszIn );
//! calculate length needed to store normal text as XML
extern UINT TextToXMLLength( const TCHAR* pszIn );
//! convert text to XML formated text (felxible because the sent object that has information 
//! about special formating)
extern void TextToXMLEx( const TCHAR* pszIn, Texts* pTexts, gd::CString* pstringXML, UINT uFlags = 0 );
//! convert XML text into string buffer as normal text
extern void XMLToText( const TCHAR* pszIn, gd::CString* pstringXML );
//! convert XML to normal text and return resulting string
extern gd::CString XMLToText( const TCHAR* pszIn );
//! convert text to string that has valid HTML characters
extern void TextToHTML( const TCHAR* pszIn, gd::CString* pstringXML );
//! replace XML characters ( <>"& )
extern void ReplaceXML( TCHAR* pszXML, TCHAR chReplace );
//! convert binary to hexstring
extern void BinToHex( const BYTE* pbValue, TCHAR* pszHex, UINT uLength );
//! convert hex string to binary
extern int  HexToBin( const TCHAR* pszHex, BYTE* pbValue, UINT uLength, TCHAR chEnd );
//! calculate the length for binary buffer needed to store
extern UINT HexToBinLength( const TCHAR* pszHex, TCHAR chEnd );
...
public:
   ...
   CXMLItem( const TCHAR* pszValue ) { m_sType = eTSTR; m_V.valpsz = pszValue; }
   CXMLItem( const gd::CString& string ) { m_sType = eTSTR; m_V.valpsz = (const TCHAR*)string; }
   ...
   operator const TCHAR*() { return m_V.valpsz; }
...
struct attribute
{
   attribute( const TCHAR* pszName, const CXMLItem& Item ) : m_pszName( pszName ), m_Item( Item ) {}
...
\code
   CFile file;
   file.Open( _T("one.xml"), CFile::modeWrite|CFile::modeCreate );
...
\code
   ...
   Archive << Child << _T("ROOT");
   Archive << Child << _T("DATA") << Attribute << _T("CHAR") << (char)'A' 
                                  << Attribute << _T("SHORT") << (short)100
                                  << Attribute << _T("LONG") << (long)12345678
                                  << Attribute << _T("DOUBLE") << (double)1234.00998877 << EndNode;
	Archive << Child << _T("TEXT") << _T("This is a sample how you can serialize data like the ")
                                     _T("MFC CArchive class");
   Archive << End;\endcode
<br>
...
public:
   CXMLArchive( STREAM* pSTREAM, UINT uState = eRead, UINT uBufferSize = 4096, TCHAR* pszBuffer = NULL );
	virtual ~CXMLArchive();
...
public:
   ...
   CXMLArchive& operator>>(TCHAR*& psz) {
...
public:
   const TCHAR* GetBuffer() {
      return (const TCHAR*)m_stringBuffer; }
   STREAM* GetStream() {
      return m_pSTREAM; }
...
public:
   ...
   void NewChild( const TCHAR* pszName );
   //! Add sibling to current node (WRITE)
   void NewSibling( const TCHAR* pszName );
   ...
   void SetHeader( const TCHAR* pszText );
   //! set attribute for node
   void SetAttribute( const TCHAR* pszName, const TCHAR* pszValue );
   //! set value for node
   ...
   int  GetBinaryValue( const TCHAR* pszName, BYTE* pBuffer, int iLength ) const;
   //! return number of attributes at current node
   unsigned GetAttributeCount() const;
   //! reads attributename from string pointer.
   static void ReadAttributeName( const TCHAR* pszNameStart, gd::CString* pstringName );
   //! get the name for attribute
   gd::CString GetAttributeName( unsigned uIndex ) const;
   //! get value or attribute from node
   int  GetAttribute( const TCHAR* pszName, TCHAR* pszBuffer, int iMaxLength ) const;
   gd::CString GetAttribute( const TCHAR* pszName ) const;
   //! get the attribute value for attribute at specified index in active node
   int  GetAttribute( unsigned uIndex, TCHAR* pszBuffer, int iMaxLength ) const;
   gd::CString GetAttribute( unsigned uIndex ) const;
   //! find attribute name in node
   int  FindAttribute( const TCHAR* pszName ) const;
   //! set string pointers to start of attribute names in buffer. Pointers are not valid when a new node has been read
   unsigned MakeAttributeIndex( TCHAR const ** ppszIndex, unsigned uCount );
   //! return item as specified type
   HRESULT GetItem( const TCHAR* pszName, CXMLItem& item );
   //! read next node
   bool ReadNext();
   //! read next node
   bool ReadNext( const TCHAR* pszName );
...
// CHECKING
   ...
   bool CompareName( const TCHAR* pszName );
...
protected:
   ...
   int _GetName( TCHAR* pszBuffer, int iMaxLength, const gd::CString* pstringNode ) const;
   gd::CString _GetName( const gd::CString* pstringNode ) const;
   int _GetAttribute( bool bText, const TCHAR* pszName, BYTE* pBuffer, int iMaxLength ) const;
   // copy attribute at position in buffer for current node (position has to point at the " character for attribute)
   int _CopyValue( bool bText, const TCHAR* pszPosition, BYTE* pBuffer, int iMaxLength ) const;
   int _GetAttributeSize( bool bText, const TCHAR* pszName ) const;
   int _GetAttributeSize( bool bText, unsigned uIndex ) const;
   int _FindAttribute( unsigned uIndex ) const;   
   void _SkipTag();
   bool _CompareText( TCHAR* pszText, int iLength );
   static bool _CompareText( CXMLArchive* pThis, TCHAR* pszText, int iLength );
...
public:
   ...
   TCHAR* m_pszStart;            //!< start for buffer
   TCHAR* m_pszPosition;         //!< current position in buffer
   TCHAR* m_pszMax;              //!< last position in buffer that is valid
   TCHAR* m_pszStop;             //!< marks the end and stop read after this
...
template<class STREAM>
inline void CXMLArchive<STREAM>::_Out( TCHAR ch ) {
...
// construct
template<class STREAM>
CXMLArchive<STREAM>::CXMLArchive( STREAM* pSTREAM, UINT uState, UINT uBufferSize, TCHAR* pszBuffer )
{
   _CHECK( pSTREAM != NULL );
   ...
   if( m_pszStart == NULL ) 
   {
      ...
      m_pszStart = new TCHAR[uBufferSize];
   }
...
template<class STREAM>
inline bool CXMLArchive<STREAM>::HasChildren() {
   return ( m_uTagState & eHasChild ) ? true : false; }

template<class STREAM>
inline bool CXMLArchive<STREAM>::HasNextChildren() {
   return ( m_uTagStateNext & eHasChild ) ? true : false; }

/////////////////////////////////////////////////
//! check if it is a end tag
template<class STREAM>
inline bool CXMLArchive<STREAM>::IsEndTag() {
   return ( m_uTagState & eEndTag ) ? true : false; }

template<class STREAM>
inline bool CXMLArchive<STREAM>::IsNextEndTag() {
   return ( m_uTagStateNext & eEndTag ) ? true : false; }

/////////////////////////////////////////////////
//! add a new child node
template<class STREAM>
void CXMLArchive<STREAM>::NewChild( const TCHAR* pszName /*!< name of node that is added*/ )
{
...
/*! create new sibling for node. This means that the created node has the same parent as
   the prevoius node. */
template<class STREAM>
void CXMLArchive<STREAM>::NewSibling( const TCHAR* pszName /*!< name of the sibling*/ )
{
   _CHECK( m_uState & eWrite );
   ...
   if( m_uTagState != 0 )
   {
      ...
      if( m_uTagState & eValue )
      {
		if(m_uState & eFormat){
         ...
         m_stringBuffer << _T("</") << m_stringsStack.Pop() << _T(">");
      }
      else
      {
...
template<class STREAM>
inline void CXMLArchive<STREAM>::NextItem( Operation operation /*!< the next type of operation*/ ) {
   _CHECK( (m_uTagState & eNextMask) == 0 );
...
template<class STREAM>
void CXMLArchive<STREAM>::SetHeader( const TCHAR* pszText /*!< header text */ )
{
   _CHECK( m_uState & eWrite );
   _CHECK_S( pszText );
   _CHECK( m_uTagState == 0 );
   m_stringBuffer << _T("<?xml ") << pszText << _T("?>");
}

/////////////////////////////////////////////////
//! set attribute name and value
template<class STREAM>
void CXMLArchive<STREAM>::SetAttribute( const TCHAR* pszName, /*!< name for attribute*/ 
                                        const TCHAR* pszValue /*!< value for attribute */ )
{
   _CHECK( m_uState & eWrite );
   _CHECK_S( pszName );
   ...
   m_stringBuffer << _T(" ") << pszName;
   if( pszValue != NULL )
   {
      _CHECK_S( pszValue );
      m_stringBuffer.Add( _T("=\"") );
...
template<class STREAM>
inline void CXMLArchive<STREAM>::SetValue( const TCHAR* pszValue ) {
   _SetValue( (const unsigned char*)pszValue, -1 ); 
}

template<class STREAM>
inline void CXMLArchive<STREAM>::SetBinaryValue( const BYTE* pbValue, int iLength ) {
   _CHECK( iLength >= 0 );
...
void CXMLArchive<STREAM>::_SetValue( const unsigned char* pValue, int iLength )
{
   ...
   if( iLength == -1 )
   {
      _CHECK_S( (const TCHAR*)pValue );
      TextToXML( (const TCHAR*)pValue, &m_stringBuffer );
   }
   else
   {
...
HRESULT CXMLArchive<STREAM>::SkipHeader()
{
   ...
   while( !((ch = _In()) == -1) ) 
   {
      if( (ch > _T(' ')) && (ch != _T('<')) ) return E_FAIL;
      
      if( ch == _T('<') ) break;

      if( ch == _T('\0') ) return S_OK;
   }
   ...
   do
   {
      if( ch == _T('<') )
      {
   	   ...
   	   if( m_pszPosition >= m_pszMax )
         {
            ...
            // at start.
            ch = _T('<');    // set ch "hard". This is later checked in order to se if we have moved in buffer.
            *m_pszStart = ch;
            m_pszStart++;
            m_uBufferSize--;
            Flush();
         ...
         if( *m_pszPosition != _T('?') && *m_pszPosition != _T('!') )
         {
   ...
   memmove( m_pszStart, m_pszPosition, uLength * sizeof(TCHAR) );
...
//! return level
template<class STREAM>
inline unsigned CXMLArchive<STREAM>::GetSection() {
   return m_stringsStack.GetCount();
}
...
template<class STREAM>
inline int CXMLArchive<STREAM>::GetName( TCHAR* pszBuffer, int iMaxLength ) const {
...
template<class STREAM>
inline int CXMLArchive<STREAM>::GetNextName( TCHAR* pszBuffer, int iMaxLength ) const {
   return _GetName( pszBuffer, iMaxLength, &m_stringNext );
}

template<class STREAM>
int CXMLArchive<STREAM>::_GetName( TCHAR* pszBuffer, int iMaxLength, const gd::CString* pstringNode ) const
{
   ...
   if( pszBuffer == NULL ) pszEnd = (TCHAR*)-1;
   else pszEnd = pszPosition + iMaxLength - 1;

   while( *pszPosition >= _T('0') && pszPosition != pszEnd ) pszPosition++;

   iLength = pszPosition - pstringNode->GetBuffer();
   if( pszBuffer != 0 )
   {
      memcpy( pszBuffer, pstringNode->GetBuffer(), iLength * sizeof(TCHAR) );
      pszBuffer[iLength] = 0;
   }
...
//! return the node name
template<class STREAM>
inline gd::CString CXMLArchive<STREAM>::GetName() const {
...
template<class STREAM>
inline gd::CString CXMLArchive<STREAM>::GetNextName() const {
...
template<class STREAM>
int CXMLArchive<STREAM>::GetValue( TCHAR* pszBuffer, int iMaxLength ) const
{
   TCHAR* pszPosition;
   TCHAR* pszCheck;
   ...
   // find value
   while( *pszPosition != 0 && *pszPosition != _T('>') ) pszPosition++;

   if( *pszPosition != _T('>') ) return -1;
   pszCheck = pszPosition - 2;
   while( *pszCheck <= _T(' ') ) pszCheck--;
   if( *pszCheck == _T('/') )
   {
      return -1;
   }
...
gd::CString CXMLArchive<STREAM>::GetValue() const
{
   ...
   // find value
   while( *pszPosition != 0 && *pszPosition != _T('>') ) pszPosition++;
   if( *pszPosition != _T('>') ) return stringValue;
...
template<class STREAM>
inline int CXMLArchive<STREAM>::GetBinaryValue( const TCHAR* pszName, BYTE* pBuffer, int iLength ) const {
...
unsigned CXMLArchive<STREAM>::GetAttributeCount() const {
   ...
   uCount = 0;
   pszPosition = m_stringBuffer.GetBuffer();
   while( *pszPosition != 0 && *pszPosition != _T('>') ) 
   {
      if( *pszPosition == _T('\"') ) uCount++;
      pszPosition++;
   }
...
//! attribute.
template<class STREAM>
inline void CXMLArchive<STREAM>::ReadAttributeName( const TCHAR* pszNameStart, gd::CString* pstringName ) {
   pstringName->Single();
   while( (*pszNameStart > _T(' ')) && (*pszNameStart != _T('=')) )
   {
...
gd::CString CXMLArchive<STREAM>::GetAttributeName( unsigned uIndex ) const {
   ...
   if( iPosition != -1 )
   {
      ...
      // go back to "="
      while( (pszPosition != pszStart) && (*pszPosition != _T('=')) ) pszPosition--;
      pszPosition--;
      // go back to first character
      while( (pszPosition != pszStart) && (*pszPosition < _T(' ')) ) pszPosition--;
      // go back to start av name
      while( (pszPosition != pszStart) && ((*pszPosition > _T(' ')) && (*pszPosition != _T('"')))  ) pszPosition--;
      pszPosition++;

      // start copy
      while( (*pszPosition > _T(' ')) && (*pszPosition != _T('=')) )
      {
...
template<class STREAM>
inline int CXMLArchive<STREAM>::GetAttribute( const TCHAR* pszName, TCHAR* pszBuffer, int iMaxLength ) const {
   return _GetAttribute( true, pszName, (BYTE*)pszBuffer, iMaxLength );
}

template<class STREAM>
gd::CString CXMLArchive<STREAM>::GetAttribute( const TCHAR* pszName ) const
{
   gd::CString string;
...
template<class STREAM>
int CXMLArchive<STREAM>::GetAttribute( unsigned uIndex, TCHAR* pszBuffer, int iMaxLength ) const
{
   int iPosition = _FindAttribute( uIndex );
   if( iPosition != -1 )
   {
...
template<class STREAM>
int CXMLArchive<STREAM>::_GetAttribute( bool bText, const TCHAR* pszName, BYTE* pBuffer, int iMaxLength ) const
{
   _CHECK( iMaxLength > 0 );
...
template<class STREAM>
int CXMLArchive<STREAM>::_CopyValue( bool bText, const TCHAR* pszPosition, BYTE* pBuffer, int iMaxLength ) const
{
   TCHAR chEnd = 0;
   ...
   if( bText == true )
   {
      return CopyXMLText( pszPosition, (TCHAR*)pBuffer, iMaxLength, chEnd );
   }
...
template<class STREAM>
int CXMLArchive<STREAM>::_GetAttributeSize( bool bText, const TCHAR* pszName ) const
{
   int iPosition;
   TCHAR chEnd;
   const TCHAR* pszPosition;
...
int CXMLArchive<STREAM>::_GetAttributeSize( bool bText, unsigned uIndex ) const
{
   ...
   if( bText == true )
   {
      return XMLToTextLength( pszPosition, _T('"') );
   }

   return HexToBinLength( pszPosition, _T('"') );
}
...
template<class STREAM>
int CXMLArchive<STREAM>::FindAttribute( const TCHAR* pszName ) const
{
   _CHECK_S( pszName );
   ...
   // move past name
   while( *pszPosition != 0 && *pszPosition > _T(' ') ) pszPosition++;
   if( *pszPosition == 0 || *pszPosition == _T('>') ) return -1;
...
template<class STREAM>
unsigned CXMLArchive<STREAM>::MakeAttributeIndex( TCHAR const **  ppszIndex, unsigned uCount )
{
   unsigned uIndex;
   const TCHAR* pszPosition;
   ...
   // move past name
   while( *pszPosition != 0 && *pszPosition > _T(' ') ) pszPosition++;
   if( *pszPosition == 0 || *pszPosition == _T('>') ) return 0;
   ...
   while( uCount )
   {
      ...
      // Move to name
      while( *pszPosition != 0 && *pszPosition <= _T(' ') ) pszPosition++;
      if( *pszPosition == 0 || *pszPosition == _T('>') ) return uIndex;
...
template<class STREAM>
HRESULT CXMLArchive<STREAM>::GetItem( const TCHAR* pszName,/*!< name of the attribute that value is read from*/
                                      CXMLItem& item /*! reference to item that gets value */ )
{
   ...
   if( pszName == NULL )
   {
      ...
      while( *pszPosition != 0 && *pszPosition != _T('>') && *pszPosition != _T('"') ) pszPosition++;
      chSave = *pszPosition;
   }
   else
   {
...
template<class STREAM>
inline bool CXMLArchive<STREAM>::ReadNext() {
   return Read();
}
...
template<class STREAM>
bool CXMLArchive<STREAM>::ReadNext( const TCHAR* pszName ) {
   unsigned uLength = _tcslen( pszName );
   while( Read() )
   {
      if( IsEndTag() ) continue;
      if( _tcsncmp( pszName, m_stringBuffer, uLength ) == 0 && m_stringBuffer[uLength] <= _T(' ') ) return true;
   }
...
//! read next item in XML text
template<class STREAM>
inline bool CXMLArchive<STREAM>::PreReadNext() {
   m_uState |= ePreRead;
   return Read();
}
 ...
 */
template<class STREAM>
bool CXMLArchive<STREAM>::CompareName( const TCHAR* pszName )
{
   const TCHAR* pszPosition, * pszStart;
   pszStart = pszPosition = m_stringBuffer;

   if( *pszPosition != *pszName ) return false;

   while( *pszPosition >= _T('0') ) pszPosition++;
   ...
   if( pThis->m_uTagState != 0 )
   {
      if( pThis->m_uTagState & (eValue|eHasChild) )
      {
		if(pThis->m_uState & eFormat){
         ...
         pThis->m_stringBuffer << _T("</") << pThis->m_stringsStack.Pop() << _T(">");
      }
      else
      {
         pThis->m_stringsStack.Pop();
         pThis->m_stringBuffer << _T("/>");
      }
      pThis->Write();
...
{
   if((m_pszPosition > m_pszStop) || (*m_pszPosition == _T('\0'))) return true;
   return false;
}
...
void CXMLArchive<STREAM>::Flush()
{
   ...
   if( m_uState & eRead )
   {
      ...
      TCHAR* pszStart = m_pszStart;
      if( m_stringInFront.GetLength() )
      {
         _tcscpy( m_pszStart, m_stringInFront );
         uBufferSize -= m_stringInFront.GetLength();
         pszStart += m_stringInFront.GetLength();
         m_stringInFront.Empty();
      }
      uBytes = 0;
      m_pSTREAM->Read( pszStart, uBufferSize * sizeof(TCHAR), &uBytes );
      m_pszMax = (TCHAR*)((UINT)pszStart + uBytes);
      if( uBytes < (uBufferSize * sizeof(TCHAR)) )
      {
         m_pszStop = m_pszMax - 1;
		 if( uBytes != 0)
			_ASSERT( *m_pszStop < 1 || *m_pszStop > 8 ); // No crummy characters
         if( m_pszStop < m_pszStart ) m_pszStop = m_pszStart;
         if( uBytes == 0 ) *m_pszStart = _T('\0');          // no read!! then set end
      }
   }
   else
   {
...
#define _xmlarchive_COPYVALUE 2
   ...
   if( m_pszPosition >= m_pszMax ) Flush();
   if( *m_pszPosition == _T('\0') ) return false;
   ...
   for(;;)
   {
      while( *m_pszPosition && *m_pszPosition != _T('<') )
      {
         m_pszPosition++;
	      if( m_pszPosition >= m_pszMax ) Flush();
         if( *m_pszPosition == _T('\0') )
         {
   ...
   if( ch == _T('/') )
   {
      ...
      while( *m_pszPosition > _T(' ') && *m_pszPosition != _T('>') )
      {
      ...
      if( m_stringsStack.GetCount() == 0 || !(*pstring == m_stringsStack.GetString( m_stringsStack.GetCount() - 1 ) ) )
      {
         ...
         {
   #ifdef TRACE
            TRACE( _T("%s "), (const TCHAR*)m_stringsStack.GetString( u ) ); 
   #endif
         }
#endif
      }
      ...
      while( (*m_pszPosition != _T('>')) && *m_pszPosition )
      {
         m_pszPosition++;
	      if( m_pszPosition >= m_pszMax ) Flush();
      }
   ...
   {
      //*check for "![CDATA["
      if( _CompareText( this, _T("<![CDATA["), 9 ) == true )
      {
   ...
   while( *m_pszPosition && *m_pszPosition != _T('<') )
   {
      ...
      if( *m_pszPosition == _T('>') )
      {
         ...
         while( *pszPosition <= _T(' ') ) pszPosition--;
         if( *pszPosition == _T('/') )
         {
            m_stringsStack.Pop();
            return true;
         ...
         m_pszPosition++;
	      if( m_pszPosition >= m_pszMax ) Flush();
         //*check for "![CDATA["
         if( (*m_pszPosition == _T('<')) && (_CompareText( this, _T("<![CDATA["), 9 ) == true) )
         {
start_cdata:
            *puState |= eCDATA;
            //* move to charcters
...
copy_cdata:
   ...
   ch = 0;
   while( *m_pszPosition && (*m_pszPosition <= _T(' ') || *m_pszPosition == _T('<')) )
   {
      m_pszPosition++;
   	if( m_pszPosition >= m_pszMax )
      {
...
template<class STREAM>
bool CXMLArchive<STREAM>::_CompareText( CXMLArchive<STREAM>* pThis, TCHAR* pszText, int iLength )
{
   bool bOk;
   int iRead;
   ...
   if( iRead > 0 )
   {
      ...
      {
         unsigned uBytes;
         pThis->m_pSTREAM->Read( pThis->m_stringInFront.GetBuffer( iRead + 1), (iRead + 1) * sizeof(TCHAR), &uBytes );
         assert( uBytes <= ((unsigned)(iRead + 1) / sizeof(TCHAR)) );
         pThis->m_stringInFront.GetBufferSetLength( uBytes / sizeof(TCHAR) );
      }
      else
      {
         unsigned uBytes;
         unsigned uStart = pThis->m_stringInFront.GetLength();
         pThis->m_pSTREAM->Read( (pThis->m_stringInFront.GetBuffer( iRead + 1 ) + uStart), (iRead - uStart + 1) * sizeof(TCHAR), &uBytes );
         assert( uBytes <= ((unsigned)(iRead + 1) / sizeof(TCHAR)) );
         pThis->m_stringInFront.GetBufferSetLength( uStart + (uBytes / sizeof(TCHAR)) );
      }
   ...
   bOk = (_tcsncmp( pThis->m_pszPosition, pszText, iLength - iRead ) == 0);
   if( iRead > 0 && bOk )
   {
      bOk = (_tcsncmp( pThis->m_stringInFront, &pszText[iLength - iRead], iRead ) == 0);
   }
...
void CXMLArchive<STREAM>::Write()
{
   ...
   {
      unsigned uIndex = 0;
      while( uIndex < m_stringBuffer.GetLength() )
      {
         if( m_pszPosition >= m_pszMax )
         {
   ...
   {
      memcpy( m_pszPosition, m_stringBuffer.GetBuffer(), m_stringBuffer.GetLength() * sizeof(TCHAR) );
#ifdef _DEBUG
      // * check crap characters
      for( unsigned u = 0; u < (unsigned)m_stringBuffer.GetLength(); u++ )
      {
         if( (unsigned char)m_pszPosition[u] < _T(' ') )
         {
            if( m_pszPosition[u] > _T('\t') )
            {
               _ASSERT( m_pszPosition[u] == _T('\n') );
            }
            else 
            {
               _ASSERT( m_pszPosition[u] == _T('\t') );
...
bool CXMLArchive<STREAM>::ValidateActiveTag()
{
   ...
   if( m_stringBuffer.Count( _T(">") ) != 1 ) return false;
   if( m_stringBuffer.Count( _T("<") ) != 0 ) return false;
   if( (m_stringBuffer.Count( _T("\"") ) % 2) == 1 ) return false;
...
template<class STREAM>
inline gd::CString CXMLArchive<STREAM>::GetActiveTagText() {
