...
The overhead created by using this smartpointer class is non. All functions are inline and it
...
template<class ITEM>
inline CStackPtr<ITEM>::~CStackPtr() {
   delete m_ppITEM;
}

template<class ITEM>
inline UINT CStackPtr<ITEM>::GetCount() {
   return m_uCount; }

template<class ITEM>
inline void CStackPtr<ITEM>::SetGrowBy( UINT uGrowBy ) {
...
inline CBits::CBits( unsigned uItems ) 
{
#ifdef _DEBUG
   m_puBitTable = NULL;
#endif
...
inline bool CBits::CheckBit( unsigned uPosition ) {
...
inline void CBits::SetBitCount( unsigned uItems ) 
{
   _ASSERT( m_puBitTable == NULL ); 
   unsigned uAllocSize = 1 + (uItems / (sizeof(unsigned) * 8));
   m_uTableSize = uItems;
   m_puBitTable = new unsigned[uAllocSize];
...
/*! \brief stores pointers in array 
   ...
   delete that objects that are stored but it does not delete pointer's when
...
template<class ITEM>
inline CArrayPtr<ITEM>::~CArrayPtr() {
   delete m_ppITEM;
}

template<class ITEM>
inline CArrayPtr<ITEM>& CArrayPtr<ITEM>::operator=( const CArrayPtr<ITEM>& other ) {
   Copy( &other );
   return *this;
}
...
*/
template<class ITEM>
inline ITEM*& CArrayPtr<ITEM>::operator[](UINT uIndex) const { 
...
template<class ITEM>
inline UINT CArrayPtr<ITEM>::GetSize() const {
   return m_uCount; }

template<class ITEM>
inline UINT CArrayPtr<ITEM>::GetCount() const {
   return m_uCount; }

template<class ITEM>
inline void CArrayPtr<ITEM>::SetGrowBy( UINT uGrowBy ) {
   _ASSERT( (uGrowBy < 0xff000000) && (uGrowBy > 0) ); // realistic
   m_uGrowBy = uGrowBy;
}
...
void CArrayPtr<ITEM>::Copy( const CArrayPtr<ITEM>* pACopyFrom ) {
   ...
   for( uIndex = 0; uIndex < m_uCount; uIndex++ ) {
      m_ppITEM[uIndex] = new ITEM( *pACopyFrom->GetAt( uIndex ) );
   }
}
...
template<class ITEM>
inline ITEM* CArrayPtr<ITEM>::GetAt( UINT uIndex ) const {
   _ASSERT( uIndex < m_uCount );
   return m_ppITEM[uIndex];
}
...
template<class ITEM>
inline void CArrayPtr<ITEM>::RemoveAll() {
   m_uCount = 0;
}
...
template<class ITEM, class KEY>
inline CSortedArrayPtr<ITEM, KEY>::CSortedArrayPtr( UINT uCount ) : CArrayPtr<ITEM>( uCount ) {
}

template<class ITEM, class KEY>
inline CSortedArrayPtr<ITEM, KEY>::~CSortedArrayPtr() {
}
...
template<class ITEM, class KEY>
inline void CSortedArrayPtr<ITEM, KEY>::AddLast( const ITEM* pITEM ) {
...
CBlocks<BLOCK>::CBlocks( const CBlocks<BLOCK>& other ) {
   ...
   m_pBLOCK    = (BLOCK*) new BYTE[sizeof(BLOCK) * m_uMaxCount];
...
template<class BLOCK>
inline BLOCK& CBlocks<BLOCK>::operator[]( UINT uIndex ) const {
   _ASSERT( uIndex < m_uCount );
   return m_pBLOCK[uIndex];
}

template<class BLOCK>
inline const CBlocks<BLOCK>& CBlocks<BLOCK>::operator+=(const BLOCK& block) {
   Add( &block );
   return *this;
}
...
template<class BLOCK>
inline UINT CBlocks<BLOCK>::GetSize() const {
   return m_uCount; }

template<class BLOCK>
inline void CBlocks<BLOCK>::SetSize( UINT uSize ) {
   _ASSERT( uSize <= m_uMaxCount );
   m_uCount = uSize; 
}

template<class BLOCK>
inline void CBlocks<BLOCK>::Add( BLOCK block ) {
   Add( &block );
}
...
template<class BLOCK>
inline BLOCK* CBlocks<BLOCK>::GetAt( UINT uIndex ) const {
   _ASSERT( uIndex < m_uCount );
   return &m_pBLOCK[uIndex];
}

template<class BLOCK>
inline void CBlocks<BLOCK>::SetAt( UINT uIndex, const BLOCK* pBLOCK ) {
   _ASSERT( uIndex < m_uCount );
   memcpy( &m_pBLOCK[uIndex], pBLOCK, sizeof( BLOCK ) );
}

template<class BLOCK>
inline void CBlocks<BLOCK>::Push( const BLOCK* pBLOCK ) {
   Add( pBLOCK ); 
}
...
template<class BLOCK>
inline BLOCK* CBlocks<BLOCK>::Data() {
   return m_pBLOCK; }

template<class ITEM>
inline void CBlocks<ITEM>::RemoveAll() {
   m_uCount = 0;
}
...
void CBlocks<BLOCK>::SafeSize( UINT uBlocks ) {
   ...
   if( uBlocks > m_uMaxCount )
   {
      ...
      pBLOCK = (BLOCK*) new BYTE[sizeof(BLOCK) * m_uMaxCount];
...
template<class BLOCK, class KEY>
inline CSortedBlocks<BLOCK, KEY>::CSortedBlocks( UINT uCount ) : CBlocks<BLOCK>( uCount ) {
}

template<class BLOCK, class KEY>
inline bool CSortedBlocks<BLOCK, KEY>::Add( const BLOCK& block ) {
   return Add( &block ); 
}
...
template<class BLOCK, class KEY>
inline void CSortedBlocks<BLOCK, KEY>::AddLast( const BLOCK* pBLOCK ) {
...
template<class ITEM>
inline void* CDListPtr<ITEM>::GetFirst() {
   return m_pFirst; }

template<class ITEM>
inline void* CDListPtr<ITEM>::GetLast() {
   return m_pLast; }

/*! get item for node en move to next node in list */
template<class ITEM>
inline ITEM* CDListPtr<ITEM>::GetNext( void*& pNode ) {
   _ASSERT( pNode != NULL );
   Node* pN;
...
template<class ITEM>
inline ITEM* CDListPtr<ITEM>::GetPrevious( void*& pNode ) {
   _ASSERT( pNode != NULL );
   Node* pN;
   pN = (CDListPtr<ITEM>::Node*)pNode;
   pNode = pN->pPrevious;
...
template<class ITEM>
inline ITEM* CDListPtr<ITEM>::GetItem( void* pNode ) {
   _ASSERT( pNode != NULL );
...
public:
   ...
   //! Detaches buffer and clear all members (doesn't delete data)
   TYPE* Detach();
...
template<class TYPE, TYPE end>
inline CBuffer<TYPE,end>::~CBuffer() {
   if( m_pTYPE != &CBuffer<TYPE,end>::m_TEnd ) delete m_pTYPE;
}

template<class TYPE, TYPE end>
inline CBuffer<TYPE,end>::operator const TYPE*() {
   return m_pTYPE; }

template<class TYPE, TYPE end>
inline CBuffer<TYPE,end>::operator TYPE*() {
   return m_pTYPE; }

template<class TYPE, TYPE end>
inline TYPE& CBuffer<TYPE,end>::operator[](UINT uIndex) const	{ 
   _ASSERT( uIndex < m_uCount );
   return m_pTYPE[uIndex]; 
}

template<class TYPE, TYPE end>
inline const CBuffer<TYPE,end>& CBuffer<TYPE,end>::operator+=(const TYPE type) {
   Add( type ); return *this;
}

template<class TYPE, TYPE end>
inline const CBuffer<TYPE,end>& CBuffer<TYPE,end>::operator+=(TYPE* pTYPE) {
   Add( pTYPE ); return *this;
}

template<class TYPE, TYPE end>
inline const CBuffer<TYPE,end>& CBuffer<TYPE,end>::operator+=(const TYPE* pTYPE) {
   Add( pTYPE ); return *this;
}
...
template<class TYPE, TYPE end>
inline UINT CBuffer<TYPE,end>::GetSize() const {
   return m_uCount; 
}

template<class TYPE, TYPE end>
inline UINT CBuffer<TYPE,end>::GetBufferSize() {
   return m_uMaxCount; 
}

template<class TYPE, TYPE end>
inline void CBuffer<TYPE,end>::SetGrowBy( UINT uGrow ) {
   m_uGrowBy = uGrow;
}
...
template<class TYPE, TYPE end>
inline TYPE CBuffer<TYPE,end>::GetAt( UINT uIndex ) const {
   _ASSERT( uIndex < m_uCount );
   return m_pTYPE[uIndex]; 
}

template<class TYPE, TYPE end>
inline void CBuffer<TYPE,end>::SetAt( UINT uIndex, TYPE type ) {
   _ASSERT( uIndex < m_uCount );
   m_pTYPE[uIndex] = type;
}
...
template<class TYPE, TYPE end>
inline TYPE CBuffer<TYPE,end>::Peek() {
   _ASSERT( m_uCount > 0 );
...
template<class TYPE, TYPE end>
inline TYPE* CBuffer<TYPE,end>::Data() {
   return m_pTYPE; 
}

template<class TYPE, TYPE end>
inline TYPE* CBuffer<TYPE,end>::Copy() {
   long lIndex;
   TYPE* pTYPE,* pT;
   lIndex = m_uCount;
   pT = m_pTYPE;
   pTYPE = new TYPE[lIndex+1];
   while( lIndex >= 0 )
   {
...
template<class TYPE, TYPE end>
inline void CBuffer<TYPE,end>::RecalculateLengthFromEnd() {
   TYPE* pTYPE = m_pTYPE;
...
template<class TYPE, TYPE end>
inline void CBuffer<TYPE,end>::Empty() {
   SetLength( 0 );
}
...
template<class TYPE>
inline CItems<TYPE>::~CItems() {
   delete [] m_pTYPE;
}

template<class TYPE>
inline CItems<TYPE>::operator const TYPE*() {
   return m_pTYPE; }

template<class TYPE>
inline TYPE* CItems<TYPE>::operator[](UINT uIndex) const	{ 
   return GetAt( uIndex ); 
}

template<class TYPE>
inline const CItems<TYPE>& CItems<TYPE>::operator+=(TYPE type) {
   Add( type ); return *this;
}
...
template<class TYPE>
inline UINT CItems<TYPE>::GetSize() const {
   return m_uCount; 
}

template<class TYPE>
inline UINT CItems<TYPE>::GetBufferSize() {
   return m_uMaxCount; 
}

template<class TYPE>
inline void CItems<TYPE>::SetGrowBy( UINT uGrow ) {
   m_uGrowBy = uGrow;
}
...
template<class TYPE>
inline TYPE* CItems<TYPE>::GetAt( UINT uIndex ) const {
   _ASSERT( uIndex < m_uCount );
   return &m_pTYPE[uIndex]; 
}

template<class TYPE>
inline void CItems<TYPE>::SetAt( UINT uIndex, TYPE type ) {
   _ASSERT( uIndex < m_uCount );
   m_pTYPE[uIndex] = type;
}
...
template<class TYPE>
inline TYPE* CItems<TYPE>::Data() {
   return m_pTYPE; 
}

template<class TYPE>
inline void CItems<TYPE>::Empty() {
   SetLength( 0 );
}
...
void CItems<TYPE>::SafeSize( UINT uLength ) {
   ...
   {
      m_uMaxCount = m_uGrowBy + uLength;
      pTYPE = (TYPE*)new BYTE[sizeof(TYPE) * m_uMaxCount];
      memcpy( pTYPE, m_pTYPE, sizeof(TYPE) * m_uCount );
      delete [] m_pTYPE;
      m_pTYPE = pTYPE;
   }
...
template<class TYPE>
inline UINT CIndex<TYPE>::GetSize() const {
   return m_uCount; }

template<class TYPE>
inline TYPE& CIndex<TYPE>::operator[]( UINT uIndex ) {
   return GetAt( uIndex ); }

template<class TYPE>
inline void CIndex<TYPE>::operator+=( TYPE type ) {
   Add( type ); 
}
...
template<class TYPE>
inline TYPE& CIndex<TYPE>::GetAt( UINT uIndex ) {
   _ASSERT( uIndex < m_uCount );
   return m_pTYPE[uIndex];
}
...
template<class ITEM, class SPLIT, SPLIT delimiter>
inline UINT CBuffers<ITEM, SPLIT, delimiter>::GetSize() const {
   return m_uItemCount; 
} 


template<class ITEM, class SPLIT, SPLIT delimiter>
inline UINT CBuffers<ITEM, SPLIT, delimiter>::GetLastOffset() {
   return m_uCount; }

template<class ITEM, class SPLIT, SPLIT delimiter>
inline void CBuffers<ITEM, SPLIT, delimiter>::SetGrowBy( UINT uGrow ) {
   m_uGrowBy = uGrow; 
}
...
template<class ITEM, class SPLIT, SPLIT delimiter>
inline ITEM* CBuffers<ITEM, SPLIT, delimiter>::GetItem( UINT uOffset ) const {
...
template<class ITEM, class SPLIT, SPLIT delimiter>
inline void CBuffers<ITEM, SPLIT, delimiter>::Empty() {
   delete (void*)m_pITEM;
   m_uItemCount = 0;
   m_uCount     = 0;
   m_uMaxCount  = 0;
...
template<class ITEM, class SPLIT, SPLIT delimiter>
inline UINT CBuffersEx<ITEM, SPLIT, delimiter>::GetCount() {
   return m_BIndex.GetSize();
}

template<class ITEM, class SPLIT, SPLIT delimiter>
inline void CBuffersEx<ITEM, SPLIT, delimiter>::SetGrowBy( UINT uGrow ) {
   m_uGrowBy = uGrow; 
}
...
template<class ITEM, class SPLIT, SPLIT delimiter>
inline ITEM* CBuffersEx<ITEM, SPLIT, delimiter>::GetItem( UINT uIndex ) {
   _ASSERT( (uIndex == 0) || (SPLIT)m_pITEM[m_BIndex[uIndex]-1] == delimiter );
...
template<class KEY, class ITEM>
inline UINT CMapPtr<KEY,ITEM>::GetCount() const {
   return m_uCount; 
}
 ...
 * \return number of items */
template<class KEY, class ITEM>
inline UINT CMapPtr<KEY,ITEM>::GetSize() const {
   return m_uCount; 
}
...
void CMapPtr<KEY,ITEM>::InitHashTable( UINT uSize ) {
   ...
   m_puTable = new UINT[uSize*2];
   m_ppEntry  = new Entry*[uSize];
...
void CMapPtr<KEY,ITEM>::Empty( bool bDelete ) {
   ...
   while( uBranch != 0 )
   {
      ...
      if( bDelete )
      {
         ...
         {
            uIndex--;
            delete m_ppEntry[uBranch][uIndex].m_pITEM;
         }
      }
      delete m_ppEntry[uBranch];
   }
   delete m_puTable;
   m_puTable = NULL;
   delete m_ppEntry;
...
template<class KEY, class ITEM>
inline void CMapPtr<KEY,ITEM>::DeleteItems() {
   Empty( true );
}
...
template<class KEY, class ITEM>
inline UINT CMapPtr<KEY,ITEM>::HashKey( KEY key ) const {
#ifdef _DEBUG
   UINT uEntry;
...
template<class ITEM>
inline ITEM& CArray<ITEM>::operator[](UINT uIndex) const	{ 
   return GetAt( uIndex ); }

template<class ITEM>
inline UINT CArray<ITEM>::GetSize() const {
   return m_uCount; 
}

template<class ITEM>
inline void CArray<ITEM>::SetGrowBy( UINT uGrowBy ) {
   _ASSERT( (uGrowBy < 0xff000000) && (uGrowBy > 0) ); // realistic
   m_uGrowBy = uGrowBy;
}
...
template<class ITEM>
inline ITEM& CArray<ITEM>::GetAt( UINT uIndex ) const {
   _ASSERT( uIndex < m_uCount );
...
template<class ITEM>
inline void CArray<ITEM>::SetAt( UINT uIndex, const ITEM& item ) {
   _ASSERT( uIndex < m_uCount );
   m_pITEM[uIndex] = item;
}
...
#endif
   DestroyItems<ITEM>( m_pITEM, m_uMaxCount );
   if( m_pITEM ) delete [] (BYTE*)m_pITEM;
   m_pITEM = 0;
   m_uMaxCount = 0;
   m_uCount = 0;
}
...
void CArray<ITEM>::SafeSize( UINT uSlots ) {
   ...
   {
      m_uMaxCount += m_uGrowBy + uSlots;
      pITEM = (ITEM*) new BYTE[sizeof(ITEM) * m_uMaxCount]; ;
      memcpy( pITEM, m_pITEM, sizeof(ITEM) * m_uCount );
...
template<class ITEM, class KEY>
inline CSortedArray<ITEM, KEY>::CSortedArray( UINT uCount ) : CArray<ITEM>( uCount ) {
}

template<class ITEM, class KEY>
inline CSortedArray<ITEM, KEY>::~CSortedArray() {
}
...
template<class ITEM, class KEY>
inline void CSortedArray<ITEM, KEY>::AddLast( const ITEM& item ) {
...
*/
template<class FIRST, class SECOND> 
inline bool __cdecl operator==(const Pair<FIRST, SECOND>& A, const Pair<FIRST, SECOND>& B) {
...
*/
template<class FIRST, class SECOND> 
inline bool __cdecl operator<(const Pair<FIRST, SECOND>& A, const FIRST& first) {
...
*/
template<class tINDEX, class tVALUE>
inline bool BinarySearch( tINDEX First, tINDEX Last, const tVALUE& Value )
{
   _ASSERT( First <= Last );
