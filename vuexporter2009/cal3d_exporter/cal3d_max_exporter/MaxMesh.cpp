...
CVertexCandidate *CMaxMesh::GetVertexCandidate(CSkeletonCandidate *pSkeletonCandidate, int faceId, int faceVertexId)
{
	...
	/*m_pIMesh->mapVerts(MAP_SHADING);
	m_pIMesh->setVCDisplayData(MAP_SHADING, NULL, NULL);*/

	/*int ncv=m_pIMesh->getNumVertCol();
	if( ncv > 0)
	{*/
		/*VertColor *vertColArray = m_pIMesh->vertColArray;
		int vidx=m_pIMesh->vcFace[faceId].t[faceVertexId];
		VertColor	color3 = vertColArray[vidx];
		DWORD color = ((int)(255.0*color3.x) << 16) + ((int)(255.0*color3.y) << 8) + (int)(255.0*color3.z);
		pVertexCandidate->SetColor( color);*/
	/*	DWORD *tt = cf[faceId].t;
		VertColor	color3 = cv[tt[faceVertexId]];
		DWORD color = ((int)(255.0*color3.x) << 16) + ((int)(255.0*color3.y) << 8) + (int)(255.0*color3.z);
		if( color != 0x00FFFFFF)
			int ii=0;
		pVertexCandidate->SetColor( color);
