...
BOOL CFileEx::Open( LPCTSTR lpFile, DWORD dwCreateDisp, DWORD dwAccess, DWORD dwShare, LPSECURITY_ATTRIBUTES lpSec )
{
	...
	m_hFile = ::CreateFile( lpFile, dwAccess, dwShare, lpSec, dwCreateDisp, 0/*FILE_FLAG_OVERLAPPED*/, NULL );
...
DWORD CFileEx::Read( BYTE* pBuffer, DWORD dwSize )
{
	...
	if( pBuffer ) {
		bRet = ::ReadFile( m_hFile, pBuffer, dwSize, &dwRead, NULL/*&ov*/ );
...
//	LONG	lOut;
	...
	if( ( lToMove <= ( LONGLONG )GetFileSize() ) ) {
			...
			( dwMoveFrom == FILE_CURRENT ) )
		{
			...
			if( (ulPos = SetFilePointer( m_hFile, lIn, NULL/*&lOut*/, dwMoveFrom )) != 0xFFFFFFFF ) {
				//ulPos = lOut;
			}else{
				ThrowErr( ::GetLastError(), FALSE );
			}
...
Cal3DComString CFileEx::GetFilePath()
{
	return Cal3DComString(m_lpFile);
}

void CFileEx::Write( const TCHAR* pszBuffer, UINT uCount, UINT* puCount )
{
	assert(false);
  /*m_uIndex += uCount;
...
UINT CFileEx::Read( TCHAR* pszBuffer, UINT uCount, UINT* puCount )
{
	*puCount = Read((BYTE*)pszBuffer, uCount);
	return *puCount;
}

LPVOID CFileEx::get_file_data(Cal3DComString fname, LPVOID *data, DWORD* size)
{
	try
	{
		if( !Open( fname))	return NULL;
	}catch( CFileExException	e)
	{
		return NULL;
	}
	Read((BYTE*)(*data = malloc( *size=GetFileSize())), GetFileSize());
	return *data;
}

//LPVOID CFileEx::get_file_data(const Cal3DComString& fname, LPVOID* data, DWORD* size)
//{
//	return get_file_data(Cal3DComString(fname), data, size);
//}

//////////////////////////////////////////////////////////////////////////
// реализация natura3d::ifile
LPVOID CFileEx::get_file_data(LPVOID *data, DWORD* size)
{
	Read((BYTE*)(*data = malloc( *size=GetFileSize())), GetFileSize());
	return *data;
}