...
bool CMaterialCandidate::Create(CBaseMaterial *pMaterial)
{
	...
	for(mapId = 0; mapId < mapCount; mapId++)
	{
		...
		Map map;
		CString sInternalName( pMaterial->GetMapFilename(mapId).c_str());
		sInternalName.MakeLower();
		if( sInternalName.Find(".psd")!=-1)
			sInternalName.Replace(".psd", ".png");
		map.strFilename = sInternalName;
#ifdef MAX3DS_BUILD
		map.type = pMaterial->GetMaxMapType(mapId);
#endif
...
//----------------------------------------------------------------------------//
// Get the emissive color of the material candidate                            //
//----------------------------------------------------------------------------//

#ifdef MAX3DS_BUILD
void CMaterialCandidate::GetEmissiveColor(float *pColor)
{
	m_pMaterial->GetEmissiveColor(pColor);
}
#endif
...
//----------------------------------------------------------------------------//
bool	CMaterialCandidate::_Map::operator<(const _Map& mp)
{
	if(type == Cal::BUMP_TEXTURE)	return true;
	if(type<mp.type && mp.type!=Cal::BUMP_TEXTURE)	return true;
	return false;
}