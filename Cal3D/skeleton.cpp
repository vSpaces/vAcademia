...
bool CalSkeleton::create(CalCoreSkeleton *pCoreSkeleton)
{
  ...
  m_pFrameBone = MP_NEW( CalFrameBone);
...
void CalSkeleton::destroy()
{
  ...
  for(iteratorBone = m_vectorBone.begin(); iteratorBone != m_vectorBone.end(); ++iteratorBone)
  {
    ...
    MP_DELETE( *iteratorBone);
  }
  m_vectorBone.clear();

  if(m_pFrameBone)
	  MP_DELETE( m_pFrameBone);
...
CalBone *CalSkeleton::getBone(LPCTSTR	name)
{
	...
	int boneID = getCoreSkeleton()->getCoreBoneIdSubName( name);
	
	if (Cal::NO_SUCH_BONE_ID == boneID)
	{
		return NULL;
	}
...
void CalSkeleton::attachToSkel(LPCTSTR aname, CalSkeleton *pOwner, CalVector trn, CalQuaternion rtn)
{
	...
	/*pSlaver->*/m_pOwnerBone = pOwner->getBone( pOwner->m_pCoreSkeleton->getCoreBoneId( name.c_str()));
...
float CalSkeleton::getFloor()
{
	...
	for(iteratorBone = m_vectorBone.begin(); iteratorBone != m_vectorBone.end(); ++iteratorBone)
	{
		std::string boneName = (*iteratorBone)->getCoreBone()->getName();
