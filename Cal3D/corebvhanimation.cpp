...
void CalCoreBvhAnimation::initialize(CalSkeleton* skeleton)
{
	...
	CalBone*	pBipBone = skeleton->getBone("bip");
	if( pBipBone)
	{
	...
	for (iter; iter != tracks.end(); iter++)
	{
		...
		std::string	boneName = (*iter)->getCoreBoneName();
		CalBone* pBone = skeleton->getBone(boneName.c_str());
		if (pBone)
		{
			...
			framesCount = max( framesCount, (int)keyFrames.size());
			int frameIndex = 0;
	...
	for( int iframe=0; iframe<framesCount; iframe++)
	{
		...
		for (iter; iter != tracks.end(); iter++)
		{
			std::string	boneName = (*iter)->getCoreBoneName();
			CalBone* pBone = skeleton->getBone(boneName.c_str());
			if (!pBone || pBone->getParentId() == Cal::NULL_PARENT_BONE_ID)	continue;
	...
	CalQuaternion	xbackRotation(CalVector(1,0,0), -1.5707963267948966192313216916398f);
	CalQuaternion	z90Rotation(CalVector(0,0,1), -1.5707963267948966192313216916398f);

	iter = tracks.begin();
	for (iter; iter != tracks.end(); iter++)
	{
		CalBone* pBone = skeleton->getBone((*iter)->getCoreBoneName().c_str());
		if (pBone && pBone->getParentId()==Cal::NULL_PARENT_BONE_ID)
		{
			std::map<float, CalCoreKeyframe*> keyFrames = (*iter)->getMapCoreKeyframe();
