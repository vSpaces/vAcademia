...
public:
	void saveState();
	void setUpState();
  virtual void blendState(float weight, const CalVector& translation, const CalQuaternion& rotation);
...
//  void calculateState(CalQuaternion qRotation=CalQuaternion(CalVector(0, 0, 0), 0), CalVector vTranslation=CalVector(0, 0, 0));
  void calculateState(bool isKinect = false, bool isEndBone = false, int maxChildCount = -1);
  ...
  void setRotationAbsolute(CalQuaternion qrtn);
