...
bool CalSaver::saveCoreBones(std::ofstream& file, const std::string& strFilename, CalCoreBone *pCoreBone)
{
  ...
  int len;
  std::string	boneName = pCoreBone->getName();
  len = boneName.size() + 1;
...
bool CalSaver::saveCoreMaterial(const std::string& strFilename, CalCoreMaterial *pCoreMaterial)
{
  ...
  for(mapId = 0; mapId < mapCount; mapId++)
  {
    ...
    file.write((char *)&len, 4);
    file.write(map.strFilename.c_str(), len-1);
	//file.write(".tga", 4);
	file.write("\0", 1);
...
bool CalSaver::saveCoreMaterialXML(const std::string& strFilename, CalCoreMaterial *pCoreMaterial)
{
  ...
  if( mapCount > 0)
  {
	  ...
	  for(mapId = 0; mapId < mapCount; mapId++)
	  {
		...
		sprintf_s(buffer, "    <%s src=\"%s\">\n", mapType.c_str(), map.strFilename.c_str());
		file.write(buffer, strlen(buffer));
	  }
	  sprintf_s(buffer, "  </maps>\n");
	  file.write(buffer, strlen(buffer));
...
bool CalSaver::saveCoreSubmesh(std::ofstream& file, const std::string& strFilename, CalCoreSubmesh *pCoreSubmesh)
{
  ...
  std::vector<CalCoreSubmesh::STextureLevel >& vectorvectorTextureCoordinate = pCoreSubmesh->getVectorVectorTextureCoordinate();
  ...
  for(vertexId = 0; vertexId < vertexCount; vertexId++)
  {
    ...
    {
      CalCoreSubmesh::TxCoor& textureCoordinate = vectorvectorTextureCoordinate[textureCoordinateId].coords[vertexId];
...
bool CalSaver::saveCoreTrack(std::ofstream& file, const std::string& strFilename, CalCoreTrack *pCoreTrack)
{
  ...
  {
	int	ns = pCoreTrack->getCoreBoneName().size();
	file.write((char *)&ns, sizeof(int));
	if( ns > 0)	  file.write((char *)pCoreTrack->getCoreBoneName().c_str(), ns);
  }
...
// open the file
  ...
  for(WORD ism=0; ism<pFace->header.dwSubmeshCount; ism++){
	  ...
	  for(i=0; i<pFace->wIndexes.size(); i++){
		  ...
		  for(unsigned int j=0; j < pFace->aMaxIndexes.size(); j++){
	  ...
	  WORD	idnumperlex = (WORD)(pFace->wDeltas.size()/pFace->header.lNum);
  ...
  if( pFace->header.bFileName > 0)	{
	...
	MP_DELETE_ARR( pFace->header.pFileName);
  }
  file.close();
}
...
bool CalSaver::saveFAP(const std::string& strFilename, FACEExpression** pexprs, int count)
{
  ...
  file.write((char *)&count, sizeof(int));
	...
	for( int ig=0; ig<count; ig++)
	{
		...
		// write name
		int	fl = exprs->name.size();
		file.write((char *)&fl, sizeof(int));
		file.write((char *)exprs->name.c_str(), exprs->name.size());
