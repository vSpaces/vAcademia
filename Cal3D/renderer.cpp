...
int CalRenderer::getNormals(float *pNormalBuffer)
{
  ...
  m_pSelectedSubmesh->fill_vertices(pNormalBuffer);
  int normalCount = /*m_pSelectedSubmesh->getVertexCount();;/*/m_pModel->getPhysique()->calculateNormals(m_pSelectedSubmesh, pNormalBuffer);
...
int CalRenderer::getTextureCoordinates(int mapId, float *pTextureCoordinateBuffer, int imeshid, int isubmeshid)
{
  ...
  std::vector<CalCoreSubmesh::STextureLevel >& vvTxCoord = m_pSelectedSubmesh->getCoreSubmesh()->getVectorVectorTextureCoordinate();
  ...
  float* pBaseVertexBuffer = pTextureCoordinateBuffer;
  ...
  std::vector<CalCoreSubmesh::TxCoor>&	mapCoords = vvTxCoord[mapId].coords;
  ...
  for( register int i=0; i < textureCoordinateCount; i++){
	  ...
	  *pBaseVertexBuffer = coordIter->v;	pBaseVertexBuffer++;
...
int CalRenderer::getVertices(float **pVertexBuffer, bool isUsualOrder)
{
  // check if the submesh handles vertex data internally
  if(m_pSelectedSubmesh->hasInternalData())
  {
  ...
  //m_pSelectedSubmesh->fill_vertices(*pVertexBuffer);
  iVertCount = /*m_pSelectedSubmesh->getVertexCount();/*/m_pModel->getPhysique()->calculateVertices(m_pSelectedSubmesh, *pVertexBuffer, isUsualOrder);
...
bool CalRenderer::selectMeshSubmesh(const int& meshId, const int& submeshId)
{
  ...
  if(m_pSelectedSubmesh == 0) return false;
  m_dwSelectedSubmesh = (WORD)submeshId;
  m_dwSelectedMesh = (WORD)meshId;
...
int CalRenderer::getTangents(int mapID, float *pVertexBuffer)
{
	// submesh does not handle the vertex data internally, so let the physique calculate it now
	int iVertCount=0;
	if( (int)m_pSelectedSubmesh->getCoreSubmesh()->getVectorVectorTangentSpace().size() <= mapID)
		return 0;

	if(m_pSelectedSubmesh->getCoreSubmesh()->isTangentsEnabled(mapID))
	{
		iVertCount = m_pModel->getPhysique()->calculateTangentSpaces(m_pSelectedSubmesh, mapID,(float *)pVertexBuffer);
	}
	return iVertCount;
}
...
WORD CalRenderer::getWireIndices(WORD** ppIndices, int imeshid, int isubmeshid)
{
	...
	*ppIndices = MP_NEW_ARR( WORD, vectorEdge.size()*2*sizeof(WORD));
	WORD*	pIndices = *ppIndices;
	int count=0;

	for( itEdge=vectorEdge.begin(); itEdge!=vectorEdge.end(); itEdge++)
	{
		*pIndices = itEdge->iv1 + (WORD)m_pSelectedSubmesh->vbStartIndex;	pIndices++;
		*pIndices = itEdge->iv2 + (WORD)m_pSelectedSubmesh->vbStartIndex;	pIndices++;
		count++;
	}

	return (WORD)vectorEdge.size();
}
...
void CalRenderer::getVBs(IDirect3DVertexBuffer8 **, IDirect3DVertexBuffer8 **, int , int )
{
	assert(false);
	/*if( imeshid==-1 || isubmeshid==-1)
	{
...
bool CalRenderer::isMouseOn( POINT /*apoint*/, D3DMATRIX	*, D3DMATRIX	*, D3DMATRIX	*, DWORD		, DWORD		)
{
...
bool CalRenderer::CLine::IsVertical(float tolerance){
	if( tolerance==0.0)
		...
		float angle = (float)arctg( p0.x-p1.x, p0.y-p1.y);
		if( angle < 0) angle*=-1;
		if( angle > 3.14f /*180*/)
			angle = angle - 3.14f;
		if( angle <= 1.57f + tolerance*3.14f/180.0f || angle >= 1.57f + tolerance*3.14f/180.0f)
			return !IsSingular();
		else
			return false;
	}
...
bool CalRenderer::CLine::IsHorizontal(float tolerance){
	if( tolerance==0.0)
		...
		if( angle > 1.57f /*90*/)
			angle = 3.14f - angle;
		if( angle < tolerance*3.14f/180.0f)
			return !IsSingular();
		else
			return false;
	}
...
void CalRenderer::get_FAP_ctrl_points(LPVOID *array, DWORD &count)
{
	...
	for( i=0; i<MAX_CONTROL_GROUP; i++)
	{
		...
		for( int in=0; in<MAX_CONTROL_POINTS_INGROUP; in++)
		{
			...
			for( unsigned int isp=0; isp<cp->points.size(); isp++)
			{
...
bool CalRenderer::create_influences(int istep, CMpeg4FDP *face)
{
	...
	int*	free_vertices = MP_NEW_ARR(int, submeshvcount[mc-1]);
		...
		if(iistep==1)
		{
			for( int ig=0; ig<MAX_CONTROL_GROUP; ig++)
			{
				for( int in=0; in<MAX_CONTROL_POINTS_INGROUP; in++)
				{
					...
					for( unsigned int i=0; i<pcp->points.size(); i++)
					{
...
//			std::vector<CalCoreSubmesh::Edge>&	edges = coresmesh->getVectorEdge();
			...
			for(unsigned int in=0; in<pneibs->nbrs.size(); in++)
			{
				...
				if(free_vertices[submeshvcount[fp.submesh_id]+nbrnum.id] != -1)	// если такая точка уже была, то
				{
					...
					for( unsigned int ia=0; ia<face->infl_array.size(); ia++)
					{
						...
						for(unsigned int iai=0; iai<arfp->influences.size(); iai++)
						...
						if( !samesparents){
							...
							for( unsigned int icnbr=0; icnbr<pnbrs->nbrs.size(); icnbr++)
							{
		...
		if(face->lastindex==(int)face->infl_array.size() || istep != -1)	break;
	}
	//
	if( istep!=-1 && face->lastindex!=(int)face->infl_array.size())
		return true;
	...
	for( ig=0; ig<MAX_CONTROL_GROUP; ig++)
	{
		for( int in=0; in<MAX_CONTROL_POINTS_INGROUP; in++)
		{
			if( !face->n_points[ig][in])	continue;
			//
			for( unsigned int i=0; i<face->n_points[ig][in]->points.size(); i++)
			{
	...
	for(unsigned int i=0; i<face->infl_array.size(); i++)
	{
		control_point*	cp;
		submesh_face_point*	fp;
