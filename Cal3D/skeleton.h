...
protected:
  ...
  MP_VECTOR<CalBone *> m_vectorBone;
...
public:
	...
	void destroy();
	__forceinline CalBone *getBone(int boneId)
	{
		if( boneId == Cal::PROJECTION_BONE_ID){	/*m_bFrameMoved=m_pFrameBone?true:false;	*/return m_pFrameBone;}
		else  if( (unsigned int)boneId < m_vectorBone.size())	return m_vectorBone[boneId];
		else	return NULL;
	}

  int	getBoneId(CalBone *apBone);
  __forceinline CalBone* get_frame(){ return m_pFrameBone;}
