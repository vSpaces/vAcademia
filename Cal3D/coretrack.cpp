...
CalCoreTrack::CalCoreTrack():
	MP_STRING_INIT(m_coreBoneName),
	MP_MAP_INIT(m_mapCoreKeyframe)
{
  m_coreBoneId = Cal::NO_SUCH_BONE_ID;
...
void CalCoreTrack::destroy()
{
  ...
  destroyMapCoreKeyframe();
  m_coreBoneId = Cal::NO_SUCH_BONE_ID;
}

void CalCoreTrack::destroyMapCoreKeyframe()
{
...
bool CalCoreTrack::getState(float time, float duration, CalVector& translation, CalQuaternion& rotation)
{
  ...
  /*std::map<float, CalCoreKeyframe *>::iterator aa = m_mapCoreKeyframe.end();
  aa--;
  std::map<float, CalCoreKeyframe *>::iterator bb = m_mapCoreKeyframe.begin();
  aa->second->setRotation( bb->second->getRotation());*/
...
void CalCoreTrack::copyData(CalCoreTrack *pTrack, float trackDur, CalCoreKeyframe* apFirstKeyframe, CalCoreKeyframe* apLastKeyframe, float a_dwKeepLast, bool bRootTrack, bool bReverse)
{
  ...
  for(iteratorCoreKeyframe = pTrack->m_mapCoreKeyframe.begin(); iteratorCoreKeyframe != pTrack->m_mapCoreKeyframe.end(); ++iteratorCoreKeyframe){
	  CalCoreKeyframe	*pkf = MP_NEW( CalCoreKeyframe);
...
{
	CalCoreKeyframe* newkey = MP_NEW( CalCoreKeyframe);
	newkey->create();
...
void CalCoreTrack::fitKeyPosition(CalVector& keytrn, CalQuaternion& keyrtn, const CalVector& frameTrn, const CalQuaternion& frameRtn)
{
	std::map<float, CalCoreKeyframe *>::iterator iteratorCoreKeyframe;
