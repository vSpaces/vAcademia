...
CalCoreSkeleton::CalCoreSkeleton():
	MP_VECTOR_INIT(m_vectorCoreBone),
	MP_LIST_INIT(m_listRootCoreBoneId)
{
}
...
int CalCoreSkeleton::addCoreBone(CalCoreBone *pCoreBone)
{
  ...
  if(pCoreBone->getParentId() == Cal::NULL_PARENT_BONE_ID)
  {
...
{
	if( !strName)	return Cal::NO_SUCH_BONE_ID;
  int boneId;
  for(boneId = 0; boneId < (int)m_vectorCoreBone.size(); boneId++)
  {
    ...
    if( _paramname == _bonename) return boneId;*/
	  std::string boneName(m_vectorCoreBone[boneId]->getName());
	  if(boneName == strName)
	  {
		  return boneId;
	  }
  ...
  return Cal::NO_SUCH_BONE_ID;
}

int CalCoreSkeleton::getCoreBoneIdSubName(LPCTSTR strName)
{
	if( !strName)	return Cal::NO_SUCH_BONE_ID;
	std::string boneName( strName);
	boneName = StringToLower(boneName);
	...
	for(boneId = 0; boneId < (int)m_vectorCoreBone.size(); boneId++)
	{
		...
		std::string	bn = m_vectorCoreBone[boneId]->getName();
		bn = StringToLower(bn);
		if( bn.find(boneName,0) != -1) return boneId;
	}
...
bool CalCoreSkeleton::containBone(const std::string& strName)
{
  int boneId;
  std::string	tofind = strName;
  tofind = StringToLower(tofind);  
  for(boneId = 0; boneId < (int)m_vectorCoreBone.size(); boneId++)
  {
	  std::string	bn = m_vectorCoreBone[boneId]->getName();
	  bn = StringToLower(bn);	  
	  if(bn.find(tofind,0) != -1) return true;
  }
  return false;
}
...
{
	if( !containBone("L Foot"))	return 0.0f;
	if( !containBone("L Calf"))		return 0.0f;
	CalCoreBone	*thighBone=NULL, *calfBone=NULL;
	thighBone = getCoreBone(getCoreBoneIdSubName("L Foot"));
	calfBone = getCoreBone(getCoreBoneIdSubName("L Calf"));
