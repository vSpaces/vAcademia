...
CalCoreAnimation *CalLoader::loadCoreAnimation(const std::string& strFilename)
{
  ...
  CalCoreAnimation *pCoreAnimation = MP_NEW( CalCoreAnimation);
  if(pCoreAnimation == 0)
  {
...
if( file_version >= 1007)
{
	...
	{
		float	len;
		file.read((char *)&len, sizeof(float));
...
CalCoreBone *CalLoader::loadCoreBones(std::ifstream& file, const std::string& strFilename)
{
  ...
  char *strBuffer;
  strBuffer = MP_NEW_ARR( char, len);
  file.read(strBuffer, len);
  std::string strName;
  strName = strBuffer;
  MP_DELETE_ARR( strBuffer);
  ...
  CalCoreBone *pCoreBone = MP_NEW( CalCoreBone);
  if(pCoreBone == 0)
  {
    CalError::setLastError(CalError::MEMORY_ALLOCATION_FAILED, __FILE__, __LINE__);
    return 0;
...
CalCoreKeyframe *CalLoader::loadCoreKeyframe(std::ifstream& file, const std::string& strFilename)
{
  ...
  CalCoreKeyframe *pCoreKeyframe = MP_NEW( CalCoreKeyframe);
  if(pCoreKeyframe == 0)
  {
    CalError::setLastError(CalError::MEMORY_ALLOCATION_FAILED, __FILE__, __LINE__);
    return 0;
...
CalCoreMaterial *CalLoader::loadCoreMaterial(const std::string& strFilename)
{
  ...
  CalCoreMaterial *pCoreMaterial = MP_NEW( CalCoreMaterial);
  if(pCoreMaterial == 0)
  {
    CalError::setLastError(CalError::MEMORY_ALLOCATION_FAILED, __FILE__, __LINE__);
    return 0;
...
CalCoreMesh *CalLoader::loadCoreMesh(const std::string& strFilename)
{
  ...
  CalCoreMesh *pCoreMesh = MP_NEW( CalCoreMesh);
  if(pCoreMesh == 0)
  {
    CalError::setLastError(CalError::MEMORY_ALLOCATION_FAILED, __FILE__, __LINE__);
    return 0;
...
CalCoreSkeleton *CalLoader::loadCoreSkeleton(const std::string& strFilename)
{
  ...
  CalCoreSkeleton *pCoreSkeleton = MP_NEW( CalCoreSkeleton);
  if(pCoreSkeleton == 0)
  {
    CalError::setLastError(CalError::MEMORY_ALLOCATION_FAILED, __FILE__, __LINE__);
    return 0;
...
CalCoreSubmesh *CalLoader::loadCoreSubmesh(std::ifstream& file, const std::string& strFilename, bool abNoSkel)
{
  ...
  CalCoreSubmesh *pCoreSubmesh = MP_NEW( CalCoreSubmesh);
  if(pCoreSubmesh == 0)
  {
    CalError::setLastError(CalError::MEMORY_ALLOCATION_FAILED, __FILE__, __LINE__);
    return 0;
...
CalCoreTrack *CalLoader::loadCoreTrack(std::ifstream& file, const std::string& strFilename)
{
  ...
  if( file_version > 1000)
  {
	...
	if( ns > 0)
	{
		name.reserve(ns);
		name.resize(ns);
	  file.read((char *)name.c_str(), ns);
	  pCoreTrack->setCoreBoneName((char *)name.c_str());
	}	
  }
...
CFace* CalLoader::loadFace(const std::string& strFilename)
{
	...
	for(WORD ism=0; ism<pFace->header.dwSubmeshCount; ism++){
		...
		WORD*	paVertIndexes = MP_NEW_ARR( WORD, dwSubmeshVertNum);
		D3DXVECTOR3*	paDeltas= MP_NEW_ARR( D3DXVECTOR3, dwSubmeshVertNum*pFace->header.lNum);
	...
	pFace->header.dwSubmeshCount = (WORD)dwRealSmc;

	file.read((char*)&(pFace->header.bFileName), sizeof(BYTE));
	if( pFace->header.bFileName > 0){
		pFace->header.pFileName = MP_NEW_ARR( char, pFace->header.bFileName+1);
...
CalCoreKeyframe *CalLoader::loadCoreKeyframe(CalMemFile &mf)
{
  ...
  //mf.read((char *)&translation.x, 12);
  /*mf.read((char *)&translation.x, 4);
  mf.read((char *)&translation.y, 4);
  mf.read((char *)&translation.z, 4);*/

  // get the rotation of the bone
  
  //mf.read((char *)&rotation.x, 4*4);
  /*mf.read((char *)&rotation.x, 4);
  mf.read((char *)&rotation.y, 4);
  mf.read((char *)&rotation.z, 4);
  mf.read((char *)&rotation.w, 4);*/

	SInputData inputData;

  mf.read((char *)&inputData, 4 + 12 + 16);
  ...
  // set all attributes of the keyframe
  pCoreKeyframe->setTime(inputData.time);
  pCoreKeyframe->setTranslation(inputData.translation);
  pCoreKeyframe->setRotation(inputData.rotation);
...
CalCoreSkeleton *CalLoader::loadCoreSkeleton(void *apData, DWORD adwSize)
{
  ...
  OutputDebugString("load skel end");
...
//  OutputDebugString(" \n");
 ...
 // OutputDebugString(tmp);
  ...
  //sprintf(tmp, "%f, %f, %f \n", txBoneSpace, tyBoneSpace, tzBoneSpace);
  //OutputDebugString(tmp);
  ...
  //sprintf(tmp, "%f, %f, %f, %f \n", rxBoneSpace, ryBoneSpace, rzBoneSpace, rwBoneSpace);
  //OutputDebugString(tmp);
...
CalCoreMesh *CalLoader::loadCoreMesh(void *apData, DWORD adwSize)
{
  ...
  if( file_version>=1007)
  {
		...
		if( edgesCount > 0)
		{
			CalShadowData*	sd = MP_NEW( CalShadowData);
...
CalCoreSubmesh *CalLoader::loadCoreSubmesh(CalMemFile &mf, bool abNoSkel)
{
  ...
  std::vector<CalCoreSubmesh::Vertex>& vertices = pCoreSubmesh->getVectorVertex();
  ...
  for (textureCoordinateId = 0; textureCoordinateId < textureCoordinateCount; textureCoordinateId++)
  {
    ...
    std::vector<CalCoreSubmesh::STextureLevel>& texCoords = pCoreSubmesh->getVectorVectorTextureCoordinate();

	std::vector<CalCoreSubmesh::TxCoor>& textureCoords = texCoords[0].coords;

  // load all vertices and their influences
  int vertexId;
  for(vertexId = 0; vertexId < vertexCount; vertexId++)
  {
    CalCoreSubmesh::Vertex& vertex = vertices[vertexId];

    // load data of the vertex
	/*mf.readType<float>(vertex.position.x);
	mf.readType<float>(vertex.position.y);
	mf.readType<float>(vertex.position.z);
    mf.readType<float>(vertex.normal.x);
    mf.readType<float>(vertex.normal.y);
    mf.readType<float>(vertex.normal.z);*/
	/*mf.read(&vertex.position, sizeof(vertex.position));
	mf.read(&vertex.normal, sizeof(vertex.normal));
    mf.readType<int>(vertex.collapseId);
    mf.readType<int>(vertex.faceCollapseCount);*/
	mf.read(&vertex.position, sizeof(vertex.position) + sizeof(vertex.normal) + sizeof(vertex.collapseId) +
		sizeof(vertex.faceCollapseCount));
   ...
   /* if(mf.eof())
    ...
    for(textureCoordinateId = 0; textureCoordinateId < textureCoordinateCount; textureCoordinateId++)
    {
      mf.read(&textureCoordinate.u, sizeof(float) + sizeof(float));

      // load data of the influence
      /*mf.readType<float>(textureCoordinate.u);
      mf.readType<float>(textureCoordinate.v);*/
	  ...
	  {
		  textureCoords[vertexId].u = textureCoordinate.u;
		  textureCoords[vertexId].v = 1.0f - textureCoordinate.v;
		//pCoreSubmesh->setTextureCoordinate(vertexId, textureCoordinateId, textureCoordinate);
    }
    }
	...
	if( !abNoSkel){
		...
		{
		  // load data of the influence
		  /*mf.readType<int>(vertex.vectorInfluence[influenceId].boneId);
		  mf.readType<float>(vertex.vectorInfluence[influenceId].weight);*/
			mf.read(&vertex.vectorInfluence[influenceId].boneId, sizeof(int) + sizeof(float));
  ...
  std::vector<CalCoreSubmesh::Face>& faces = pCoreSubmesh->getVectorFace();

  CalCoreSubmesh::Face& face = faces[0];
	#ifdef FACESIZEISWORD
	mf.read(&face.vertexId[0], sizeof(WORD) * 3 * faceCount);
	#else
  mf.read(&face.vertexId[0], sizeof(int) * 3 * faceCount);
	#endif

  return pCoreSubmesh;
}

CalCoreMaterial *CalLoader::loadFakeMaterial(const char* textureName)
{
	// allocate a new core material instance
	CalCoreMaterial *pCoreMaterial = MP_NEW( CalCoreMaterial);
	if (pCoreMaterial == 0)
	...
	pCoreMaterial->setSpecularColor(specularColor);
	pCoreMaterial->setEmissiveColor(emissiveColor);
	pCoreMaterial->setShininess(0);

	// reserve memory for all the material data
	if(!pCoreMaterial->reserve(1))
	{
	...
	CalCoreMaterial::Map map;
	map.strFilename = textureName;
	map.type = 0;    
	map.userData = 0;
    
	pCoreMaterial->setMap(0, map);
...
CMpeg4FDP *CalLoader::loadFAPU(void *apData, DWORD adwSize)
{
	...
	// read data
	CMpeg4FDP* face = MP_NEW( CMpeg4FDP);
	...
	{
		control_point*	point = MP_NEW( control_point);
...
{
	FACEExpressionsPack*	pack = MP_NEW( FACEExpressionsPack);
...
int CalLoader::loadFAP(void *apData, DWORD adwSize, FACEExpression** vector)
{
  ...
  file.read((char *)&retCount, sizeof(int));
	...
	for( int ig=0; ig<retCount; ig++)
	{
		...
		{
			MPEGExpression*	expr = MP_NEW( MPEGExpression);
			MP_NEW_V( fe, FACEExpression, expr);
		...
		{
			MPEGViseme*	viseme = MP_NEW( MPEGViseme);
			MP_NEW_V( fe, FACEExpression, viseme);
