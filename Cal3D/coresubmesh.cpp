...
CalCoreSubmesh::_STangentSpaceLevel::_STangentSpaceLevel():
	MP_VECTOR_INIT(coords)
{

}

CalCoreSubmesh::_STangentSpaceLevel::_STangentSpaceLevel(const _STangentSpaceLevel& other):
	MP_VECTOR_INIT(coords)
{
	coords = other.coords;
}

CalCoreSubmesh::_STextureLevel::_STextureLevel():
	MP_VECTOR_INIT(coords)
{
}

CalCoreSubmesh::_STextureLevel::_STextureLevel(const _STextureLevel& other):
	MP_VECTOR_INIT(coords)
{
	coords = other.coords;
}

CalCoreSubmesh::_Vertex::_Vertex():
	MP_VECTOR_INIT(vectorInfluence)
{
}

CalCoreSubmesh::_Vertex::_Vertex(const _Vertex& other):
	MP_VECTOR_INIT(vectorInfluence)
{
	position = other.position;
    normal = other.normal;
    collapseId = other.collapseId;
    faceCollapseCount = other.faceCollapseCount;
	vectorInfluence = other.vectorInfluence;
}

CalCoreSubmesh::_Neighbour::_Neighbour():
	MP_VECTOR_INIT(nbrs)
{
	
}

CalCoreSubmesh::_Neighbour::_Neighbour(const _Neighbour& other):
	MP_VECTOR_INIT(nbrs)
{
	nbrs = other.nbrs;
}
...
void CalCoreSubmesh::UpdateTangentVector(unsigned short v0, unsigned short v1, unsigned short v2, unsigned short mapId)
{
	std::vector<CalCoreSubmesh::Vertex> &vvtx = getVectorVertex();
	std::vector<CalCoreSubmesh::TxCoor> &vtex = m_vvTextCoord[mapId].coords;
	...
	if /*(*/(fabsf(prod1) < 0.000001f)/*||(fabsf(prod2) < 0.000001f))*/ return;
	...
	float component = (tangent * vvtx[v0].normal);
	tangent -= (vvtx[v0].normal * component);
	tangent.normalize();

	CalVector binormal = tangent.cross(vvtx[v0].normal);
	if ((tangent.cross(binormal)).dot(vvtx[v0].normal) < 0)
	{
		m_vectorvectorTangentSpace[mapId].coords[v0].crossFactor = -1.0f;
	}

	// Step 3: Add the estimated tangent to the overall estimate for the vertex.


	m_vectorvectorTangentSpace[mapId].coords[v0].tangent+=tangent;
}
...
bool CalCoreSubmesh::enableTangents(int mapId, bool enabled)
{
	...
	m_vectorvectorTangentSpace[mapId].coords.reserve(m_vectorVertex.size());
	m_vectorvectorTangentSpace[mapId].coords.resize(m_vectorVertex.size());

	std::vector<TangentSpace>::iterator it = m_vectorvectorTangentSpace[mapId].coords.begin();
	std::vector<TangentSpace>::iterator itEnd = m_vectorvectorTangentSpace[mapId].coords.end();
	...
	std::vector<Face>::iterator iter = m_vectorFace.begin();
	std::vector<Face>::iterator iterEnd = m_vectorFace.end();
	for ( ; iter != iterEnd; iter++)
	{
		Face& face = *iter;
		UpdateTangentVector(face.vertexId[0], face.vertexId[1], face.vertexId[2], (unsigned short)mapId);
		UpdateTangentVector(face.vertexId[1], face.vertexId[2], face.vertexId[0], (unsigned short)mapId);
		UpdateTangentVector(face.vertexId[2], face.vertexId[0], face.vertexId[1], (unsigned short)mapId);
	}

	it = m_vectorvectorTangentSpace[mapId].coords.begin();
	for( ; it != itEnd; it++)
	{
...
std::vector<CalCoreSubmesh::STangentSpaceLevel >& CalCoreSubmesh::getVectorVectorTangentSpace()
{
...
std::vector<CalCoreSubmesh::STextureLevel> & CalCoreSubmesh::getVectorVectorTextureCoordinate()
{
...
bool CalCoreSubmesh::reserve(int vertexCount, int textureCoordinateCount, int faceCount, int springCount)
{
  ...
  {
    m_vvTextCoord[textureCoordinateId].coords.reserve(vertexCount);
    m_vvTextCoord[textureCoordinateId].coords.resize(vertexCount);

	if (m_vectorTangentsEnabled[textureCoordinateId])
	{
		m_vectorvectorTangentSpace[textureCoordinateId].coords.reserve(vertexCount);
		m_vectorvectorTangentSpace[textureCoordinateId].coords.resize(vertexCount);
	}
	else
	{
		m_vectorvectorTangentSpace[textureCoordinateId].coords.clear();
	}
  }
...
bool CalCoreSubmesh::setTangentSpace(int vertexId, int textureCoordinateId, const CalVector& tangent, float crossFactor)
{
	...
	m_vectorvectorTangentSpace[textureCoordinateId].coords[vertexId].tangent = tangent;
	m_vectorvectorTangentSpace[textureCoordinateId].coords[vertexId].crossFactor = crossFactor;
	return true;
}
...
{
  if((textureCoordinateId < 0) || (textureCoordinateId >= (int)m_vvTextCoord.size())) return false;
  if((vertexId < 0) || (vertexId >= (int)m_vvTextCoord[textureCoordinateId].coords.size())) return false;

  m_vvTextCoord[textureCoordinateId].coords[vertexId] = textureCoordinate;
...
CalVector _v3 = m_vectorVertex[m_iteratorFace2->vertexId[2]].position;
		if( is_face_identity(*m_iteratorFace2))	continue;
			...
			{
				int pv1 = (int)pow(2.0f, m_iteratorFace->getVertIDnum(ind11));
				int pv2 = (int)pow(2.0f, m_iteratorFace2->getVertIDnum(ind21));
		...
		{
			if( !(m_facesEdges[if1] & (int)pow(2.0f, ie)))
			{
...
{
	int vcount=0;
	WORD	vIDs[2] = {0xFFFF, 0xFFFF};
	WORD	vIDs2[2] = {0xFFFF, 0xFFFF};
	for( int i=0; i<3; i++)
	{
		for( int j=0; j<3; j++)
		{
...
{
	CalCoreSubmesh*	submesh = MP_NEW( CalCoreSubmesh);
