...
#define DEF_VENDOR(x, y, z) m_vendorList.insert(std::map<std::string, std::string>::value_type(x, y)); m_vendorListID.insert(std::map<std::string, int>::value_type(x, z));
#define DEF_DEVICE(x, y) m_deviceList.insert(std::map<std::string, std::string>::value_type(x, y));
#define DEF_DEVICE_INV(y, x) m_deviceList.insert(std::map<std::string, std::string>::value_type(x, y));
#define DEF_NON_NPOT_SIGNATURE(x) m_nonNPOTDevices.push_back(x);
...
{
	std::string versionAtiGl = GetGLVersion();
	
	int buildVersion = 0;
	unsigned int pos = 0;
	if ((pos = versionAtiGl.find_last_of('.', versionAtiGl.length())) != 0 && pos < versionAtiGl.length())
	{
		std::string strBuild = versionAtiGl.substr(pos + 1, versionAtiGl.length() - pos - 1);
		buildVersion = atoi(strBuild.c_str());		
	}
...
bool CGPUInfo::Analyze()
{
	...
	// get vendor ID
	m_vendorStringID = id.substr(8, 4);

	// get device ID
	m_deviceStringID = id.substr(17, 4);
...
{
	std::map<std::string, std::string>::iterator iter = m_vendorList.find(m_vendorStringID);
...
{
	unsigned char* vendor = (unsigned char*)glGetString(GL_VENDOR);
	if (vendor)
	{
		return (char*)vendor;
	}
...
{
	unsigned char* renderer = (unsigned char*)glGetString(GL_RENDERER);
	if (renderer)
	{
...
{
	std::map<std::string, int>::iterator iter = m_vendorListID.find(m_vendorStringID);
...
{
	std::map<std::string, int>::iterator iter = m_vendorListID.find(vendorStringID);
...
{
	std::map<std::string, std::string>::iterator iter = m_deviceList.find(m_deviceStringID);
...
{
	std::map<std::string, std::string>::iterator it = m_vendorList.begin();
	std::map<std::string, std::string>::iterator itEnd = m_vendorList.end();
...
{
	if (extInfo.IsExtensionSupported(GL_EXT_texture_filter_anisotropic))
	{
		if (GetVendorID() == VENDOR_INTEL)
		{
			return 0;
...
void CGPUInfo::GetIntelDriverVersion(int& leadVersion, int& majorVersion, int& minorVersion)
{
	...
	leadVersion = atoi((char *)ver.substr(0, pos).c_str());
	ver = ver.substr(pos + 1, ver.size() - pos);
	pos = ver.find_first_of(".");
	majorVersion = atoi((char *)ver.substr(0, pos).c_str());
	ver = ver.substr(pos + 1, ver.size() - pos);
	pos = ver.find_first_of(".");
	minorVersion = atoi((char *)ver.substr(0, pos).c_str());
}
...
void CGPUInfo::GetAtiDriverVersion(int& majorVersion, int& minorVersion)
{
	...
	if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, currentClassesPath, 0, KEY_READ, &hKey) == ERROR_SUCCESS)
	{
		...
		{		
			strcpy(classKeyName, currentClassesPath);
			strcat(classKeyName, classKeyNameBuf);
	...
	if (driverVersionString != "")
	{
		...
		majorVersion = atoi(majorStr.c_str());
		minorVersion = atoi(minorStr.c_str());
	}
...
bool CGPUInfo::IsNPOTSupport()
{
	if (-1 == m_isSupportNPOT)
	{
		...
		if (!extInfo.IsExtensionSupported(GLEXT_arb_texture_non_power_of_two))
		{
			m_isSupportNPOT = 0;
		}
		...
		if (VENDOR_ATI == GetVendorID())
		{
			...
			std::vector<std::string>::iterator it = m_nonNPOTDevices.begin();
			std::vector<std::string>::iterator itEnd = m_nonNPOTDevices.end();
...
unsigned int CGPUInfo::GetVideoMemorySize()
{
	...
	{
		if (extInfo.IsExtensionSupported(GLEXT_ati_mem_info))
		{
			unsigned int m[4], m2[4], m3[4];
			glGetIntegerv(0x87FB /*VBO_FREE_MEMORY_ATI*/, (GLint*)&m[0]);
			glGetIntegerv(0x87FC /*TEXTURE_FREE_MEMORY_ATI*/, (GLint*)&m2[0]);
			glGetIntegerv(0x87FD /*RENDERBUFFER_FREE_MEMORY_ATI*/, (GLint*)&m3[0]);
...
bool CGPUInfo::IsSaveCompressionSupported()
{
	...
	int major, minor;
	GetDriverVersion(major, minor);

	if ((major == 9) && ((minor == 5) || (minor == 6) || (minor == 7)))
	{
		return false;
	}
...
bool CGPUInfo::IsLowEndVideocard()
{
	...
	else if (GetVendorID() == VENDOR_ATI)
	{
		...
		// Radeon HD 3xxx: < 3400
		if ((number >= 3000) && (number < 3400))
		{
			return true;
		}

		// Radeon HD 4xxx: < 4400
		if ((number >= 4000) && (number < 4400))
		{
			return true;
		}
