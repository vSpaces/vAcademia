...
int CRecognize::DoFilter(DWORD aDataSize, BYTE *aData, DWORD position, float limit)
{
	long i;


 WORD tmp=0,distance=0;
 char pr[2];
 bool minus=FALSE;
 index_time=0;
...
//  ATLTRACE("Position=%d DataSize=%d",position,aDataSize);
 ...
 {  pr[0]=aData[i]; pr[1]=aData[i+1]; 
	...
	minus=FALSE;
	if (tmp & 0x8000)	{ tmp^=0xFFFF; tmp++; minus=TRUE;  }
   ...
   // Поиск пауз и лексем
	...
	{ DWORD s=j*interval;
      temp=0;
	  if (s+interval<=index_time) 
	  { for (DWORD  k=s; k<s+interval; k++)	 temp+=time[k].value;
		temp/=interval;
	    prom=(temp*100.0)/32767.0;
  	    if (prom < limit) num_pausa++;
	    else num_lexema++;
