...
{
	double	arg = (x*second.x+y*second.y+z*second.z)/(length()*second.length());
	if( arg >= 1)
		return 0.0;
	if( arg <= -1)
		return 3.141692f;
	double resCos = acos( arg);
	return resCos;
}
...
float CalVector::dot(CalVector &v)
{
	return x*v.x+y*v.y+z*v.z;
}

CalVector CalVector::cross(CalVector &v)
{
	CalVector ret;
	ret.x = y * v.z - z * v.y;
	ret.y = z * v.x - x * v.z;
...
CalVector CalVector::cross(CalVector &v1, CalVector &v2)
{
...
bool CalVector::coplanar(CalVector &v)
{
...
{
	if( x==FLT_MAX || y==FLT_MAX || y==FLT_MAX) return true;
	return false;
}
...
CalVector CalVector::project(CalVector v)
{
...
CalVector CalVector::project(CalVector p1, CalVector p2)
{
