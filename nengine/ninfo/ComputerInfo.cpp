...
bool CComputerInfo::IsAeroEnabled()
{
	return false;
}
...
std::string CComputerInfo::GetCompName()
{
	...
	if( GetComputerName((LPTSTR)&computerName, &size))
	{
	...
	return "GetComputerName FAILED";
}

__int64 CComputerInfo::GetPhysicalMemorySize()
{
	MEMORYSTATUSEX stat;
...
__int64 CComputerInfo::GetAvailablePhysicalMemorySize()
{
	MEMORYSTATUSEX stat;
	stat.dwLength = sizeof (stat);
	GlobalMemoryStatusEx(&stat);
...
__int64 CComputerInfo::GetVirtualMemorySize()
{
	MEMORYSTATUSEX stat;
	stat.dwLength = sizeof (stat);
	GlobalMemoryStatusEx(&stat);
...
__int64 CComputerInfo::GetAvailableVirtualMemorySize()
{
	MEMORYSTATUSEX stat;
	stat.dwLength = sizeof (stat);
	GlobalMemoryStatusEx(&stat);
...
unsigned int CComputerInfo::GetRealCoreCount()
{
	_SYSTEM_INFO nfo;
	GetSystemInfo(&nfo);

	if ((nfo.dwNumberOfProcessors > 1) && (!IsMultiCoreCapable()))
	{
		nfo.dwNumberOfProcessors = 1;
	}

	return nfo.dwNumberOfProcessors;
}

void CComputerInfo::GetOSVersion(unsigned int* apuMajor,  unsigned int* apuMinor,  unsigned int* apuBuild)
{
	unsigned int dwVersion = GetVersion();

	if( apuMajor)
		*apuMajor = (DWORD)(LOBYTE(LOWORD(dwVersion)));
	if( apuMinor)
		*apuMinor = (DWORD)(HIBYTE(LOWORD(dwVersion)));
	if (apuBuild && dwVersion < 0x80000000)              
		*apuBuild = (DWORD)(HIWORD(dwVersion));
}

std::string CComputerInfo::GetOSVersionString()
{
    unsigned int dwMajorVersion = 0;
    unsigned int dwMinorVersion = 0;
    unsigned int dwBuild = 0;

	GetOSVersion( &dwMajorVersion, &dwMinorVersion, &dwBuild);
...
std::string CComputerInfo::GetOSVersionHumanString()
{
    unsigned int dwMajorVersion = 0;
    unsigned int dwMinorVersion = 0;
    unsigned int dwBuild = 0;
...
unsigned int CComputerInfo::IsMultiCoreCapable()
{
#define HWD_MT_BIT				0x10000000
#define NUM_CORE_BITS			0xFC000000

	unsigned int MaxInputValue = 0, regEAX = 0, regEDX = 0;
	...
	if (MaxInputValue >= 1)
	{
		...
		regEDX = regEDX & (unsigned int)HWD_MT_BIT;
	}

	if ((MaxInputValue >= 4) && regEDX)
	{
		__asm
		{
			mov eax, 4
	...
	return ((regEAX & NUM_CORE_BITS) >> 26);
}

bool CComputerInfo::IsHyperthreadingEnabled()
{
	if ((GetProcessorCount() > 1) && (IsMultiCoreCapable() == 0))
	{
		return true;
	}
...
bool CComputerInfo::IsSecondLifeInstalled()
{
	HKEY rKey;
	if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, "SOFTWARE\\Classes\\secondlife", 0, KEY_QUERY_VALUE, &rKey) != ERROR_SUCCESS)
	{
		return false;
	}
