                          ...
                          int level )
{
    ...
    fin  = FileOpenA( input, "rb" );
    fout = FileOpenA( output, "wb" );
...
int ZlibEngine::compmem( char *input,unsigned long* sizein,
                         char *output,unsigned long* sizeout,
                         int level )
{
    err = Z_OK;
...
//  err = deflate( this, Z_FINISH );
    ...
    deflateEnd( this );
    *sizeout=this->total_out ;
    return err;
}

int ZlibEngine::decompmem(char *input,unsigned long* sizein,
                          char *output,unsigned long* sizeout)                        
{
    err = Z_OK;
    next_in = (unsigned char*)input;
    ...
    err = inflate( this, Z_FULL_FLUSH);
	if (err == Z_OK && avail_out == 0)
	{
		return -2;
	}
