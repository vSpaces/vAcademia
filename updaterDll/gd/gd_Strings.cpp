   ...
   )
{
   ...
   lLength = wcslen( pszText );
   _AllocAdd( lLength );
   m_pData->AddTextLength( lLength );

   lLength++;
   lLength *= sizeof(wchar_t);
...
PURPOSE: Add text to strings object */
void CStrings::Add(
   const wchar_t* pszText, // text that is going to be added to strings object
   ...
   )
{
   ...
   lLength *= sizeof(wchar_t);
   memcpy( m_pData->End(), pszText, lLength );
   m_pData->End()[uLength * sizeof(wchar_t)] = 0;
   m_pData->AddLength( lLength );
   m_pData->m_iCount++;
}
...
const wchar_t* CStrings::GetString(
   UINT uIndex //
   ) const
{
   ...
   ) const
{
   ...
   {
      if( wcscmp( (wchar_t*)(pbCompare + sizeof(long)), pszText ) == 0 )
      {
         return iIndex;
      }
...
RETURN:  index for string or -1 if string was not found */
int CStrings::Find(
   const wchar_t* pszText, // Text that is searched for
   ...
   ) const
{
   ...
   for( iIndex = 0; iIndex < m_pData->GetCount(); iIndex++ )
   {
      if( (*(long*)pbCompare == (long)uLength) && (wcsncmp( (wchar_t*)(pbCompare + sizeof(long)), pszText, uLength ) == 0) )
      {
         return iIndex;
      }
      pbCompare = _Next( pbCompare );
...
const wchar_t* CStrings::Next(
   const wchar_t* pszString //
   ) const
{
   _ASSERT( pszString == NULL || pszString >= (wchar_t*)(m_pData->GetData() + sizeof(long)) );
   _ASSERT( pszString < (wchar_t*)m_pData->End() );
   ...
   if( pbPosition >= m_pData->End() - sizeof(wchar_t) ) return NULL;
   
   return (wchar_t*)(pbPosition + sizeof(long));
}
...
const wchar_t* CStrings::Pop()
{
   ...
   m_pData->SetLength( m_pData->GetLength() - (*(long*)pbLast * sizeof(wchar_t)) - sizeof(wchar_t) - sizeof(long) );
   m_pData->m_iCount--;
   return (wchar_t*)(pbLast + sizeof(long));
}
