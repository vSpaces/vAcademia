...
///////////////////////////////////////////////////////////////////////////////
// ctor
CMiniVersion::CMiniVersion(LPCWSTR lpszPath)
{
	ZeroMemory(m_szPath, sizeof(m_szPath)*sizeof(m_szPath[0]));

	if (lpszPath && lpszPath[0] != 0)
	{
		lstrcpynW(m_szPath, lpszPath, sizeof(m_szPath)-1);
	}
	else
	{
	}
...
BOOL CMiniVersion::Init()
{
	...
	dwSize = ::GetFileVersionInfoSizeW(m_szPath, &dwHandle);
	if (dwSize == 0)
		return FALSE;
	...
	rc = ::GetFileVersionInfoW(m_szPath, dwHandle, dwSize, m_pData);
	if (!rc)
		return FALSE;
