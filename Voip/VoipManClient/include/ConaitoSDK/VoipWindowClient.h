...
#define WM_INITVOIPSYSTEM				(WM_USER+1000)

class CVoipWindowClient : public CWindowImpl<CVoipWindowClient>
{
private:
	IVoipManWndEventHandler* manager;

public:
	CVoipWindowClient(IVoipManWndEventHandler* aManager);
	~CVoipWindowClient(void);

	BEGIN_MSG_MAP(CVoipWindowClient)
		...
		MESSAGE_HANDLER(WM_INITVOIPSYSTEM, OnInitVoipSystem)
	END_MSG_MAP()
...
protected:
	...
	LRESULT OnInitVoipSystem(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled);
};
