...
CComString::CComString(LPCSTR aCharData, int aDataLen)
{
	...
	_tcsncpy(m_pszString, aCharData, (aDataLen < 0 ? 0 : aDataLen));
}
...
{
	for (int i = GetLength() - 1;  i >= 0 && m_pszString[ i] != ch;  i--);
	return i;
}
...
void CComString::FormatV(LPCTSTR pszCharSet, va_list vl)
{
	...
	do {
		nBufferSize += 100;
		delete [] pszTemp;
		pszTemp = new TCHAR[nBufferSize];	ZeroMemory( pszTemp, nBufferSize*sizeof(TCHAR));
