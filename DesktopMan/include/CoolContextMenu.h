...
protected:
	void MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct)
	{
		...
		else
		{
			...
			if (pmd->fState & MFS_DEFAULT)
			{
				...
				hOldFont = (HFONT)::SelectObject(dc.m_hDC, fontBold);
                fontBold.DeleteObject();
			}
			else
				hOldFont = (HFONT)::SelectObject(dc.m_hDC, m_fontMenu);
    ...
    LRESULT InitMenuPopupHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
    {
        ...
        for (int i = 0; i < menuPopup.GetMenuItemCount(); i++)
        {
            ...
            {
                MenuItemData * pMI = MP_NEW( MenuItemData);
                ATLASSERT(pMI != NULL);
                ...
                if (pMI)
                {
                    ...
                    pMI->fType = mii.fType;
                    pMI->fState = mii.fState & ~MFS_DEFAULT;

                    // Associate an image with a menu item
                    static_cast<T*>(this)->AssociateImage(mii, pMI);

                    pMI->lpstrText = MP_NEW_ARR( TCHAR, lstrlen(szString) + 1);
...
public:
    ...
    void GetSystemSettings()
    {
		...
		WTL::CWindowDC dc(static_cast<T*>(this)->m_hWnd);
		HFONT hFontOld = (HFONT)::SelectObject(dc.m_hDC, m_fontMenu);
		RECT rcText = { 0, 0, 0, 0 };
