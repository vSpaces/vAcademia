...
public:
	...
	*/
	...
	static BOOL Process(CDib24* pDst, const CDib24* pSrc, POINT& ptPosDst, CRect rcSrc = CRect())
	{
		_ASSERTE(pDst->GetBitCount()==24);
		...
		DWORD nHeight = pDst->GetHeight();
		//
		pSrcBits += (pSrc->GetLineWidth()*rcSrc.top) + (rcSrc.left * pSrc->GetPixelWidth());
		pDstBits += (pDst->GetLineWidth()*ptPosDst.y) + (ptPosDst.x*pDst->GetPixelWidth());
		size_t left = max(0, ptPosDst.x);
		size_t right = min((unsigned long)ptPosDst.x + rcSrc.Width(), nWidth);
		size_t top = max(0, ptPosDst.y);
		size_t bottom = min((unsigned long)ptPosDst.y + rcSrc.Height(), nHeight);
