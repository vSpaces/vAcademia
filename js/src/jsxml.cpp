    ...
    if (capacity == 0) {
        ...
        if (array->vector) {
            if (cx)
                ...
                js_freeMP(array->vector);
        }
        vector = NULL;
    } else {
		vector = (void **)js_reallocMP((void*)array->vector, capacity * sizeof(void *));
        if (
...
XMLArrayAddMember(JSContext *cx, JSXMLArray *array, uint32 index, void *elt)
{
    ...
    if (index >= array->length) {
        if (index >= JSXML_CAPACITY(array)) {
			...
			vector = (void **)js_reallocMP(array->vector, capacity * sizeof(void *));
            if (
#if JS_BITS_PER_WORD == 32
                (size_t)capacity > ~(size_t)0 / sizeof(void *) ||
#endif
...
XMLArrayTruncate(JSContext *cx, JSXMLArray *array, uint32 length)
{
    ...
    if (length == 0) {
        ...
        vector = (void **) js_reallocMP(array->vector, length * sizeof(void *));
        if (!vector)
            return;
    }
