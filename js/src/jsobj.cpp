...
obj_toSource(JSContext *cx, uintN argc, jsval *vp)
{
    ...
    if (!chars) {
        ...
        chars = (jschar *) js_mallocMP(((outermost ? 4 : 2) + 1) * sizeof(jschar));
        nchars = 0;
        if (!chars)
            goto error;
        if (outermost)
        ...
        nchars = js_strlen(chars);
        chars = (jschar *)
            js_reallocMP((ochars = chars), (nchars + 2 + 1) * sizeof(jschar));
        if (!chars) {
            js_freeMP(ochars);
            goto error;
        }
...
#undef SAFE_ADD
            ...
            chars = (jschar *)
                js_reallocMP((ochars = chars), curlen * sizeof(jschar));
            if (!chars) {
