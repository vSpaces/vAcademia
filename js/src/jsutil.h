...
#endif
/*
static JS_INLINE void* js_mallocMP(size_t bytes) {
    if (bytes < sizeof(void*))
        bytes = sizeof(void*);
	return MP_NEW_ARR(BYTE, bytes);
//    return malloc(bytes);
}
*/

#define js_mallocMP(bytes) MP_NEW_ARR(BYTE, ((bytes) < sizeof(void *) ? sizeof(void *) : (bytes)))
/*
static JS_INLINE void* js_callocMP(size_t bytes) {
    if (bytes < sizeof(void*)) // for asyncFree 
        bytes = sizeof(void*);
	void* pData = MP_NEW_ARR(BYTE, bytes);
...
#define js_callocMP(bytes) MP_NEW_Z_ARR(BYTE, ((bytes) < sizeof(void *) ? sizeof(void *) : (bytes)))

/*
static JS_INLINE void* js_reallocMP(void* p, size_t bytes) {
    if (bytes < sizeof(void*)) // for asyncFree 
        bytes = sizeof(void*);
	...
	if (p != NULL)
	{
		size_t oldByteSize = _msize(p);
		if (pData != NULL)
			memcpy(pData, p, oldByteSize < bytes ? oldByteSize : bytes);
...
#define js_reallocMP(p, bytes) MP_REALLOC_ARR(BYTE, p, ((bytes) < sizeof(void *) ? sizeof(void *) : (bytes)))


/*
static JS_INLINE void js_freeMP(void* p) {
	if (p != NULL)
		MP_DELETE_ARR(p);
//    free(p);
}
...
#define js_freeMP(p) if (p != NULL) MP_DELETE_ARR(p)
