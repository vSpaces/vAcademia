...
js_alloc_sftbl_entry(void *priv, const void *key)
{
    ...
    return (JSHashEntry *) js_mallocMP(JS_MAX(nbytes, sizeof(JSHashEntry)));
}
...
SaveScriptFilename(JSRuntime *rt, const char *filename, uint32 flags)
{
    ...
    if (flags != 0) {
        ...
        if (!sfp) {
            ...
            sfp = (ScriptFilenamePrefix *) js_mallocMP(sizeof(ScriptFilenamePrefix));
            if (!sfp)
                return NULL;
