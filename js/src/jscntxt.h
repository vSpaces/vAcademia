...
# define ENUM_CACHE_METER(name)     ((void) 0)
    ...
    inline void* mallocMP(size_t bytes) {
        return js_mallocMP(bytes);
    }

    inline void* callocMP(size_t bytes) {
        return js_callocMP(bytes);
    }

    inline void* reallocMP(void* p, size_t bytes) {
        return js_reallocMP(p, bytes);
    }

    inline void freeMP(void* p) {
        js_freeMP(p);
    }
...
#define CX_FROM_THREAD_LINKS(tl) \
    ...
    inline void createDeallocatorTask() {
        ...
        if (runtime->deallocatorThread && !runtime->deallocatorThread->busy())
            tls->deallocatorTask = MP_NEW(JSFreePointerListTask);
    }
