...
#undef MSG_DEF
    ...
    rt = (JSRuntime *) js_mallocMP(sizeof(JSRuntime));
    if (!rt)
        return NULL;
    ...
    if (!rt->debuggerLock)
        goto bad;
    rt->deallocatorThread = MP_NEW(JSBackgroundThread);
...
"JS API usage error: %u context%s left in runtime upon JS_DestroyRuntime.\n",
    ...
    if (rt->deallocatorThread) {
		...
		MP_DELETE(rt->deallocatorThread);
    }
#endif
