...
Tracker::addTrackerPage(const void* v) {
    ...
    struct Tracker::TrackerPage* p = (struct Tracker::TrackerPage*)
        js_callocMP(sizeof(*p) - sizeof(p->map) + (TRACKER_PAGE_SIZE >> 2) * sizeof(LIns*));
    p->base = base;
    p->next = pagelist;
    pagelist = p;
    return p;
...
struct VMFragment : public Fragment
{
    VMFragment(const void* _ip, JSObject* _globalObj, uint32 _globalShape, uint32 _argc
			#if defined(NJ_VERBOSE) && defined(DEBUG)
               , uint32_t profFragID
			#endif
	) :
        Fragment(_ip 
      ...
      tempTypeMap(cx)
{
    ...
    debug_only_printf(LC_TMTreeVis, "TREEVIS RECORD FRAG=%p ANCHOR=%p\n", (void*)fragment,
                      (void*)anchor);
#endif
	MP_NEW_V(lir_buf_writer, LirBufWriter, lirbuf);
    lir = lir_buf_writer;
#ifdef DEBUG
	SanityFilter* pSanityFilterTmp = NULL;
	MP_NEW_V(pSanityFilterTmp, SanityFilter, lir);
	sanity_filter_1 = pSanityFilterTmp;
    lir = sanity_filter_1;
#endif
    debug_only_stmt(
        if (js_LogController.lcbits & LC_TMRecorder) {
			MP_NEW_P4(p, VerboseWriter, tempAlloc, lir, lirbuf->names, &js_LogController);
			verbose_filter = (LirWriter*)p;
           //MP_NEW_V4(verbose_filter, VerboseWriter, tempAlloc, lir, lirbuf->names, &js_LogController);
		   lir = verbose_filter;
        }
    )
    // CseFilter must be downstream of SoftFloatFilter (see bug 527754 for why).
	CseFilter* pCseFilterTmp = NULL;
	MP_NEW_V2(pCseFilterTmp, CseFilter, lir, tempAlloc);
	cse_filter = pCseFilterTmp;
    lir = cse_filter;
    if (nanojit::AvmCore::config.soft_float)
	{
		SoftFloatFilter* pSoftFloatFilterTmp = NULL;
		MP_NEW_V(pSoftFloatFilterTmp, SoftFloatFilter, lir);
		float_filter = pSoftFloatFilterTmp;
        lir = float_filter;
	}
	ExprFilter* pExprFilterTmp = NULL;
	MP_NEW_V(pExprFilterTmp, ExprFilter, lir);
	expr_filter = pExprFilterTmp;
    lir = expr_filter;

	FuncFilter* pFuncFilterTmp = NULL;
	MP_NEW_V(pFuncFilterTmp, FuncFilter, lir);
	func_filter = pFuncFilterTmp;
    lir = func_filter;
#ifdef DEBUG
	SanityFilter* pSanityFilterTmp2 = NULL;
	MP_NEW_V(pSanityFilterTmp2, SanityFilter, lir);
	sanity_filter_2 = pSanityFilterTmp2;
    lir = sanity_filter_2;
#endif
    lir->ins0(LIR_start);
...
#ifdef DEBUG
    debug_only_stmt( MP_DELETE(verbose_filter); )
    MP_DELETE(sanity_filter_1);
    MP_DELETE(sanity_filter_2);
#endif
    MP_DELETE(cse_filter);
    MP_DELETE(expr_filter);
    MP_DELETE(func_filter);
    MP_DELETE(float_filter);
    MP_DELETE(lir_buf_writer);
}
...
TraceRecorder::compile(JSTraceMonitor* tm)
{
    ...
    char* label = (char*)js_mallocMP((filename ? strlen(filename) : 7) + 16);
              ...
              VMSideExit* expectedInnerExit, jsbytecode* outer, uint32 outerArgc)
{
    ...
    MP_NEW_V10(tm->recorder, TraceRecorder, cx, anchor, f, ti,
    ...
    )
{
    ...
    if (!(c = anchor->target)) {
        ...
        c->root = f;
		#if defined(NJ_VERBOSE) && defined(DEBUG)
        tm->branches = new (alloc) Seq<Fragment*>(c, tm->branches);
		#endif
    }
...
js_InitJIT(JSTraceMonitor *tm)
{
    ...
    if (js_LogController.lcbits & LC_FragProfile) {
        tm->profAlloc = MP_NEW(VMAllocator);
    ...
    tm->dataAlloc = MP_NEW(VMAllocator);
    tm->tempAlloc = MP_NEW(VMAllocator);
    tm->reTempAlloc = MP_NEW(VMAllocator);
    tm->codeAlloc = MP_NEW(CodeAlloc);
    tm->flush();
	#if defined(NJ_VERBOSE) && defined(DEBUG)
		tm->branches = NULL;
	#endif
    ...
    tm->reservedDoublePoolPtr = tm->reservedDoublePool = MP_NEW_ARR(jsval, MAX_NATIVE_STACK_SLOTS);
...
#undef MONITOR_JITSTAT
    ...
    if (tm->codeAlloc) {
        MP_DELETE(tm->codeAlloc);
        tm->codeAlloc = NULL;
    }

    if (tm->dataAlloc) {
        MP_DELETE(tm->dataAlloc);
        tm->dataAlloc = NULL;
    }

    if (tm->tempAlloc) {
        MP_DELETE(tm->tempAlloc);
        tm->tempAlloc = NULL;
    }

    if (tm->reTempAlloc) {
        MP_DELETE(tm->reTempAlloc);
        tm->reTempAlloc = NULL;
    }
}
