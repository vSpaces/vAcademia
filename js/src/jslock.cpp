...
NewFatlock()
{
    JSFatLock *fl = (JSFatLock *)js_mallocMP(sizeof(JSFatLock)); /* for now */
    if (!fl) return NULL;
    fl->susp = 0;
    fl->next = NULL;
    fl->prevp = NULL;
...
js_SetupLocks(int listc, int globc)
{
    ...
    global_locks = (PRLock **) js_mallocMP(global_lock_count * sizeof(PRLock*));
    if (!global_locks)
        return JS_FALSE;
    ...
    fl_list_table = (JSFatLockTable *) js_mallocMP(i * sizeof(JSFatLockTable));
    if (!fl_list_table) {
        js_CleanupLocks();
        return JS_FALSE;
    }
